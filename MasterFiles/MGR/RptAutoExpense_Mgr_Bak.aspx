<%@ Page Language="C#" AutoEventWireup="true" CodeFile="RptAutoExpense_Mgr.aspx.cs" Inherits="MasterFiles_MR_RptAutoExpense" %>
                                    ...
                                    <asp:TemplateField HeaderText="Place of Work" ItemStyle-HorizontalAlign="Left">
                                        ...
                                        <ItemTemplate>
                                           <asp:Label ID="lblTerrName" runat="server" Text='<%# Bind("Territory_Name") %>'></asp:Label>
                                            ...
                                            </asp:DropDownList><asp:HiddenField ID="toHidden" runat="server" Value='<%# Bind("to_place") %>'/>
                                         </ItemTemplate>
                                    </asp:TemplateField>
...
<script>
    ...
    function _CalDist($x, TotDist, fare) {
        ...
        //alert($x.parentNode.children[1].value);
        $x.parentNode.children[0].value = selValue;
        var $R1 = $x.parentNode.parentNode;
        //alert(selValue);
        // var $Fr = $R.cells[0].childNodes[1].value.split("~~");
        var $To = $x.parentNode.children[1].value.split("~");
        //alert($To[0]);
        ...
        if ($To[0] == '') {
            ...
            if ($st > -1) {
                ...
                if ($R1.cells[4].children.length > 0) {
                    if ($R1.cells[4].children[0].value == 'AS-FA' || $R1.cells[4].children[0].value == 'FA' || $R1.cells[4].children[0].value == 'AE-FA' || $R1.cells[4].children[0].value == 'AA-FA' || $R1.cells[4].children[0].value == 'AA-FE'
        ...
        $Dis = $Dis * 2;
        //alert($Dis);
        var pValue = $R1.cells[6].children[1].value;
        ...
        $R1.cells[6].children[1].value = $Dis;
        $R1.cells[6].children[0].innerHTML = $Dis;
        ...
        //alert("dist "+tDis);
        var oldFare = parseFloat($R1.cells[7].children[1].innerHTML.replace(/,/g, ''));

        if ($Dis > 120) {
            $R1.cells[7].children[0].innerHTML = parseFloat(parseFloat($Dis) * parseFloat(fare));
            $R1.cells[7].children[1].value = parseFloat(parseFloat($Dis) * parseFloat(fare));
        }
        else {
            $R1.cells[7].children[0].innerHTML = parseFloat(parseFloat($Dis) * parseFloat(fare));
            $R1.cells[7].children[1].value = parseFloat(parseFloat($Dis) * parseFloat(fare));
        }
        var newFare = parseFloat($R1.cells[7].children[0].innerHTML.replace(/,/g, ''));
        ...
        var oldTot = parseFloat($R1.cells[8].childNodes[0].innerHTML.replace(/,/g, ''));
        if (isNaN(allw)) allw = 0;
        if (isNaN(newFare)) newFare = 0;
        if (isNaN(oldTot)) oldTot = 0;
        var newTot = parseFloat(newFare) + parseFloat(allw);
        $R1.cells[8].childNodes[0].innerHTML = parseFloat(newTot);
        $R1.cells[8].childNodes[1].value = parseFloat(newTot);
        //alert($R1.parentNode.children.length - 1);
        $tot = $R1.parentNode.children[$R1.parentNode.children.length - 1].cells[8].children[0];
        $totF = $R1.parentNode.children[$R1.parentNode.children.length - 1].cells[7].children[0];
        //$tot=document.getElementById("TotalVal");
        $grndTot = document.getElementById("grandTotalName");
    ...
    function onAllowTypeChange($e, flag, type) {
       ...
       // alert(flag + " :: " + $e + "typ" + type);
        ...
        var oldRowTot = parseFloat($R.cells[7].children[0].innerHTML.replace(/,/g, ''));
        var $tot = $R.parentNode.children[$R.parentNode.children.length - 1].cells[8].children[0];
       ...
       // alert(str);
        ...
        $R.cells[7].children[0].innerHTML = oldRowTot - oldAllow + newAllow;
        $R.cells[7].children[1].value = oldRowTot - oldAllow + newAllow;
    ...
    function onAllowChange($e) {
        ...
        var oldRowTot = parseFloat($R.cells[8].children[0].innerHTML.replace(/,/g, ''));
        ...
        //alert(oldAllow + " :: " + newAllow);
        $R.cells[8].children[0].innerHTML = oldRowTot - oldAllow + newAllow;
        $R.cells[8].children[1].value = oldRowTot - oldAllow + newAllow;
        $tot.innerHTML = oldTot - oldAllow + newAllow;
        $grndAllowTot.innerHTML = oldGrndAllowTot - oldAllow + newAllow;
        $grndTot = document.getElementById("grandTotalName");
        var grndVal = parseFloat($grndTot.innerHTML.replace(/,/g, ''));
    ...
    function onFareChange($e) {
        $R = $e.parentNode.parentNode;
        //alert($R);
        var oldFare = parseFloat($R.cells[6].children[1].value.replace(/,/g, ''));
        var oldRowTot = parseFloat($R.cells[8].children[0].innerHTML.replace(/,/g, ''));
        var $tot = $R.parentNode.children[$R.parentNode.children.length - 1].cells[7].children[0];
        var oldTot = parseFloat($tot.innerHTML.replace(/,/g, ''));
        var $grndFareTot = $R.parentNode.children[$R.parentNode.children.length - 1].cells[6].children[0];
        var oldGrndFareTot = parseFloat($grndFareTot.innerHTML.replace(/,/g, ''));
        // alert(oldRowTot + " :: " + oldTot);
        var newFare = parseFloat($R.cells[6].children[0].value.replace(/,/g, ''));
        if (isNaN(oldFare)) oldFare = 0;
        if (isNaN(newFare)) newFare = 0;
        //alert(newFare + " :: " + oldFare);

        $R.cells[6].children[1].value = newFare;
        $R.cells[7].children[0].innerHTML = oldRowTot - oldFare + newFare;
        $R.cells[7].children[1].value = oldRowTot - oldFare + newFare;
