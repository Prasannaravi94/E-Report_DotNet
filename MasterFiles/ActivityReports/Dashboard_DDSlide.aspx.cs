...
public partial class MasterFiles_ActivityReports_Dashboard_DDSlide : System.Web.UI.Page
{
        ...
        if (!Page.IsPostBack)
        {
            ...
            else if (Session["sf_type"].ToString() == "" || Session["sf_type"].ToString() == "3")
            {
               ...
               (UserControl_MenuUserControl)LoadControl("~/UserControl/MenuUserControl.ascx");
                ...
                Fillteam();
                //if (rblBasedOn.SelectedValue == "0")
                //{
                //    //ddlSrch.Items.FindByValue("8").Enabled = false;
                //}
            }
            ...
            rblBasedOn.Checked = true;
            if (rblBasedOn.Checked)
            {
                //FillReport();
            }
    ...
    private void FillReport()
    {
        ...
        if (rblBasedOn.Checked)
        {
            dsSlide = sf.getSlidesNew(ddlFieldForce.SelectedValue, fDate.ToString(), tDate.ToString(), div_code, 1);
            ...
            if (dsSlide.Tables[0].Rows.Count > 0)
            {
                ...
                dsSlideListedDr = sf.getSlideListedDrNew(ddlFieldForce.SelectedValue, fDate.ToString(), tDate.ToString(), div_code, 1);
                dsSlideTime = sf.getSlideTimeNew(ddlFieldForce.SelectedValue, fDate.ToString(), tDate.ToString(), div_code, 1);
                dsSlideusrLike = sf.getSlideUsrLikeNew(ddlFieldForce.SelectedValue, fDate.ToString(), tDate.ToString(), div_code, 1);
                ...
                foreach (DataRow slide in dsSlide.Tables[0].Rows)
                {
                    ...
                    var rsMSL_code = from row in dsSlideListedDr.Tables[0].AsEnumerable()
                                     ...
                                     && row.Field<string>("MSL_code") != string.Empty
                                     select new
                                     {
                    ...
                    var rsSlideUsrDisLike = from row in dsSlideusrLike.Tables[0].AsEnumerable()
                                            where row.Field<string>("SlideName") == slide["SlideName"].ToString()
                                            && row.Field<byte>("usrLike") == 2 && row.Field<string>("MSL_code") != string.Empty
                                            select new
                                            {
                                                SlideUsrLike = row.Field<byte>("usrLike")
                                            };
                    ...
                    var rsSlideUsrNil = from row in dsSlideusrLike.Tables[0].AsEnumerable()
                                        where row.Field<string>("SlideName") == slide["SlideName"].ToString()
                                        && row.Field<byte>("usrLike") == 0 && row.Field<string>("MSL_code") != string.Empty
                                        select new
                                        {
                                            SlideUsrLike = row.Field<byte>("usrLike")
                                        };
            ...
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "Alert", "<script>alert('No record found');</script>", false);
            }
            ...
            " },	" +
            " subtitle: {	" +
            "     text: 'FieldForce: " + ddlFieldForce.SelectedItem.Text + "<br />   From: " + ddlFMonth.SelectedItem.Text + " " + ddlFYear.SelectedValue + "\u00a0 \u00a0 To: " + ddlTMonth.SelectedItem.Text + " " + ddlFYear.SelectedValue + "\u00a0 \u00a0 \u00a0' " +
            " },	" +
            " xAxis: [{	" +
            "     categories:" + strSlide + ",	" +
            "     min: 0,	" +
            "     max: " + ((dsSlide.Tables[0].Rows.Count > 15) ? 15 : (dsSlide.Tables[0].Rows.Count - 1)) + ", " +
            "     crosshair: true	" +
            " }], " +
            " scrollbar: {" +
            ...
            "     gridLineWidth: 0,	" +
            "     title: {	" +
            "         text: 'Total Listed Doctors & Chemist',	" +
            "         style: {	" +
            ...
            " },	" +
            "	    xAxis: {	" +
            "	        categories: " + strSlide + ", min: 0, max:" + ((dsSlide.Tables[0].Rows.Count > 15) ? 15 : (dsSlide.Tables[0].Rows.Count - 1)) + ",	" +
            "	    },	" +
            "	    yAxis: {	" +
            ...
            {
                if (hdnbtnClicked.Value == "1")
                {
                    Page.ClientScript.RegisterStartupScript(Page.GetType(), "Alert", "<script>alert('please select base on');</script>", false);
                }
            }
        //}
    ...
    //protected void rblBasedOnChanged(object sender, EventArgs e)
    //{
    //    if (rblBasedOn.SelectedValue == "0")
    //    {
    //        ddlSrch.Items.FindByValue("8").Enabled = false;
    ...
    //    else
    //    {
    //        ddlSrch.Items.FindByValue("8").Enabled = true;
    //    }
    //}
    ...
    //    {
    //        DataSet dsSubDiv = SD.getSubdivision(div_code);
    //        if (dsSubDiv.Tables[0].Rows.Count > 0)
    //        {
    //            ddlSrc2.DataTextField = "subdivision_name";
    //            ddlSrc2.DataValueField = "subdivision_code";
    //            ddlSrc2.DataSource = dsSubDiv;
    //            ddlSrc2.DataBind();
    //        }
    //    }
    //    else
    //    {
    //        DataSet dsSubDiv = SD.getSubdivisionSF(div_code, ddlFieldForce.SelectedValue);
    //        if (dsSubDiv.Tables[0].Rows.Count > 0)
    //        {
    //            ddlSrc2.DataTextField = "subdivision_name";
    //            ddlSrc2.DataValueField = "subdivision_code";
    ...
    protected void Btnsrc_Click(object sender, EventArgs e)
    {
        ...
        //    drPrdBrd = "NULL";
        //    drTrrCode = "NULL";
        //    drSubDiv = ddlSrc2.SelectedValue;
        //    filter = " Filter: Sub Division: " + ddlSrc2.SelectedItem.Text;
        //}
    ...
    {
        string attachment = "attachment; filename=" + rblBasedOn.Text.Replace(" ", "_") + ".xls";
