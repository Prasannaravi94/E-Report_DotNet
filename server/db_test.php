...
function getVstDets() { 
	...
	$sql ="select Trans_Act_SL,Activity_Name,Exp_Name,Amount,DCR_DetID from Trans_Activity_Expense_Head where DCR_ID='" . $ACd . "' order by Activity_Date asc "; 
$arr=[];
$result=performQuery($sql);
for($j=0;$j <count($result);$j++)
{
$id = $result[$j]['Trans_Act_SL'];
$sql ="select name,value from Trans_Activity_Expense_Detail where DCR_ID='" . $id . "'"; 
 $res=performQuery($sql);
$result[$j]["exp_det"] =$res;
}

    $query = "exec spGetVstDetApp '" . $ACd . "','" . $typ . "'";
    $rst= performQuery($query);
$arr=array($rst,$result);
return $arr;
...
$nextVisitDate=$DetTB['nextVisitDate'];
	...
	if(isset($tableData["Activity_Event_Pick_Captures"]))
				{
					$Event_Pick_Captures = $tableData["Activity_Event_Pick_Captures"];
...
if(isset($tableData['Activity_CME_Report'])){
$cme=$tableData['Activity_CME_Report'];
}
if(isset($tableData['Activity_Demo_Report'])){
$demo=$tableData['Activity_Demo_Report'];
}
if(isset($tableData['Activity_install_Report'])){
$install=$tableData['Activity_install_Report'];
}
if(isset($tableData['Activity_document_Report'])){
$document=$tableData['Activity_document_Report'];
}
if(isset($tableData['Activity_lead_Report'])){
$lead=$tableData['Activity_lead_Report'];
}
if(isset($tableData['Activity_payfollow_Report'])){
$payfollow=$tableData['Activity_payfollow_Report'];
}
if(isset($tableData['Activity_promotional_Report'])){
$promotional=$tableData['Activity_promotional_Report'];
}

if(isset($tableData['activity_order_delret'])){
$deliveryReturn=$tableData['activity_order_delret'];

}

if(isset($tableData['activity_order_otattended'])){
$otAttended=$tableData['activity_order_otattended'];

}
if(isset($tableData['activity_order_create'])){
$ordercreate=$tableData['activity_order_create'];

}
if(isset($tableData['activity_order_followup'])){
$orderfollowup=$tableData['activity_order_followup'];

}
if(isset($tableData['Activity_Expense_Report'])){
$activity_Expense=$tableData['Activity_Expense_Report'];
}
...
if($cme!=null){
$sql="select isnull(max(Trans_Act_SL),0)+1 sl_no from Trans_Activity_CME_Head";
$tr=performQuery($sql);
$trans_act_sl=$tr[0][sl_no];
$desc=$cme['desc'];
$empfd=$cme['empfd'];
$venue=$cme['venue'];
$place=$cme['place'];
$act_name=$cme['name'];
$act_id=$cme['selected']['id'];
$cme_order = $cme['order'];
$cme_demo = $cme['demo'];
$cme_enquiry = $cme['enquiry'];

$sql = "insert into Trans_Activity_CME_Head select '$trans_act_sl','$sfCode',getdate(),'$act_id'," . $cCode . ",'$desc','$empfd','$venue','$place','" . $ARCd . "','" . $ARDCd . "','" . $Owndiv . "','$act_name'";
performQuery($sql);
	for ($j = 0; $j < count($cme_order); $j++) {                    
	$sql="insert into Trans_Activity_CME_Prod_Detail select '$trans_act_sl','1','Order','".$cme_order[$j]["cateid"]."','','".$cme_order[$j]["product"]."','".$cme_order[$j]["product_Nm"]."','".$cme_order[$j]["rx_qty"]."','".$cme_order[$j]["sample_qty"]."',''";  
	performQuery($sql);                     
	}
	for ($j = 0; $j < count($cme_demo); $j++) {                    
	$sql1="insert into Trans_Activity_CME_Prod_Detail select '$trans_act_sl','2','Demo','".$cme_demo[$j]["cateid"]."','','".$cme_demo[$j]["product"]."','".$cme_demo[$j]["product_Nm"]."','".$cme_demo[$j]["rx_qty"]."','".$cme_demo[$j]["sample_qty"]."',''";
	performQuery($sql1);                     
	}
	for ($j = 0; $j < count($cme_enquiry); $j++) {                    
	$sql2="insert into Trans_Activity_CME_Prod_Detail select '$trans_act_sl','3','Enquiry','".$cme_enquiry[$j]["cateid"]."','','".$cme_enquiry[$j]["product"]."','".$cme_enquiry[$j]["product_Nm"]."','".$cme_enquiry[$j]["rx_qty"]."','".$cme_enquiry[$j]["sample_qty"]."',''";
	performQuery($sql2);                     
	}
}
if($demo!=null){
$sql="select isnull(max(Trans_Act_SL),0)+1 sl_no from Trans_activity_Demo";
$tr=performQuery($sql);
$trans_act_sl=$tr[0][sl_no];
$StTime=$demo['StTime'];
$S_Time=str_replace("T", " ",$StTime);
$EndTime=$demo['EndTime'];
$E_Time=str_replace("T", " ",$EndTime);
$call_received=$demo['CallRecDt'];
$call_received_time=str_replace("T", " ",$call_received);

$WrkDn=$demo['WrkDn'];
$DStatus_id=$demo['Status']['selected']['id'];
$DStatus_name=$demo['Status']['selected']['name'];
$overdemo_id=$demo['OvrDemo']['selected']['id'];
$overdemo_name=$demo['OvrDemo']['selected']['name'];
$Demo_Remarks=$demo['Remarks'];$NxtVst=$demo['NxtVst'];
$Prod_id=$demo['Prod']['selected']['id'];$Prod_name=$demo['Prod']['selected']['name'];
$ProdAppr=$demo['ProdAppr'];$ProdPerf=$demo['ProdPerf'];$ProdQual=$demo['ProdQual'];
$OvrAllDmo=$demo['OvrAllDmo'];
$AbtDemo_id=$demo['AbtDemo']['selected']['id'];
$AbtDemo_name=$demo['AbtDemo']['selected']['name'];
$need_impro=$demo['NTImp'];$Suggest=$demo['Suggest'];$remarks=$demo['Rmks'];
$Active_id=$demo['ActState']['selected']['id'];
$Activeid_name=$demo['ActState']['selected']['name'];
$complete_id=$demo['CmpltState']['selected']['id'];
$complete_name=$demo['CmpltState']['selected']['name'];
$report_no=$demo['DRNo'];$report_date=$demo['DRDate'];
$serial_no=$demo['SLNo'];$HisCrdNo=$demo['HisCrdNo'];$con_persion=$demo['CnctPer'];
$dr_received=$demo['DRRecDt'];$history_card=$demo['HCrdUpdt'];

$sql = "insert into Trans_activity_Demo select '$trans_act_sl','$sfCode',getdate(),''," . $cCode . ",'$S_Time','$E_Time','$WrkDn','$DStatus_id','$DStatus_name','$overdemo_id','$overdemo_name','$Demo_Remarks','$NxtVst','','','$Prod_id','$Prod_name','$ProdAppr','$ProdPerf','$ProdQual','$OvrAllDmo','$AbtDemo_id','$AbtDemo_name','$need_impro','$Suggest','$remarks','$Active_id','$Activeid_name','','$complete_id','$call_received_time','$report_no','$report_date','$serial_no','$HisCrdNo','$con_persion','$dr_received','$history_card','" . $ARCd . "','" . $ARDCd . "','" . $Owndiv . "'";
performQuery($sql);
}


if($document!=null){
$sql="select isnull(max(Trans_Act_SL),0)+1 sl_no from Trans_activity_Documentaion";
$tr=performQuery($sql);
$trans_act_sl=$tr[0][sl_no];
$description=$document['description'];
$feedback=$document['feedback'];

$sql = "insert into Trans_activity_Documentaion select '$trans_act_sl','$sfCode',getdate(),'$description','$feedback'," . $cCode . ",'" . $ARCd . "','" . $ARDCd . "','" . $Owndiv . "'";
performQuery($sql);
}
if($install!=null){
$sql="select isnull(max(Trans_Act_SL),0)+1 sl_no from Trans_activity_install_acceptance";
$tr=performQuery($sql);
$trans_act_sl=$tr[0][sl_no];
$Icano=$install['icano'];$Icadate=$install['icadate'];$Dcno=$install['dcno'];$Dcdate=$install['dcdate'];
$Billno=$install['billno'];$Bldate=$install['bldate'];$Support_addr=$install['csaddr'];$Phone=$install['phone'];
$Preverified=$install['name'];$Preverified_id=$install['selected']['id'];
$Slno=$install['slno'];$Aslno=$install['aslno'];$Plistno=$install['plistno'];$Addinfo=$install['addinfo'];
$Gremark=$install['gremark'];$Annx=$install['annx'];$Fbreport=$install['fbreport'];$Rron=$install['rron'];
$Wedate=$install['wedate'];$Reviewedby=$install['reviewedby'];$Revdate=$install['revdate'];$Hcard=$install['hcard'];$Date=$install['date'];
$Packing_id = $install['packing']['selected']['id'];$Packing = $install['packing']['name'];
$Certify_id = $install['certify']['selected']['id'];$Certify = $install['certify']['name'];
$Satisfy_id = $install['satisfy']['selected']['id'];$Satisfy = $install['satisfy']['name'];

$samp = $install['Sproduct'];
$sql = "insert into Trans_activity_install_acceptance select '$trans_act_sl','$sfCode',getdate(),''," . $cCode . ",'$Icano','$Icadate','$Dcno','$Dcdate','$Billno','$Bldate','$Support_addr','$Phone','$Preverified_id','$Preverified','$Slno','$Packing_id','$Packing','$Aslno','$Plistno','$Addinfo','$Certify_id','$Certify','$Gremark','','$Annx','','$Satisfy_id','$Satisfy','$Fbreport','','$Rron','$Wedate','$Reviewedby','$Revdate','$Hcard','$Date','" . $ARCd . "','" . $ARDCd . "','" . $Owndiv . "'";
performQuery($sql);


for ($j = 0; $j < count($samp); $j++) {
		$sql="insert into Trans_activity_install_acceptance_Prod_Detail select '$trans_act_sl','".$samp[$j]["cateid"]."','','".$samp[$j]["product"]."','".$samp[$j]["product_Nm"]."','".$samp[$j]["rx_qty"]."','".$samp[$j]["sample_qty"]."',''";
		performQuery($sql);
		}

}
if($lead!=null){
$sql="select isnull(max(Trans_Act_SL),0)+1 sl_no from Trans_Activity_Lead_Head";
$tr=performQuery($sql);
$trans_act_sl=$tr[0][sl_no];
$custfd=$lead['custfd'];
$empfd=$lead['empfd'];
...
$sql = "insert into Trans_Activity_Lead_Head select '$trans_act_sl','$sfCode',getdate()," . $cCode . ",'$custfd','$empfd','$amount','" . $ARCd . "','" . $ARDCd . "','" . $Owndiv . "'";

performQuery($sql);

                    for ($j = 0; $j < count($samp); $j++) {
                    
$sql="insert into Trans_Activity_Lead_Prod_Detail select '$trans_act_sl','".$samp[$j]["cateid"]."','','".$samp[$j]["product"]."','".$samp[$j]["product_Nm"]."','','',''";
performQuery($sql);
                    }
}
if($payfollow!=null){
$sql="select isnull(max(Trans_Act_SL),0)+1 sl_no from Trans_Activity_payment_followup";
$tr=performQuery($sql);
$trans_act_sl=$tr[0][sl_no];
$feedback=$payfollow['feedback'];

$sql = "insert into Trans_Activity_payment_followup select '$trans_act_sl','$sfCode',getdate()," . $cCode . ",'$feedback','" . $ARCd . "','" . $ARDCd . "','" . $Owndiv . "'";
performQuery($sql);
}
if($promotional!=null){
$sql="select isnull(max(Trans_Act_SL),0)+1 sl_no from Trans_Activity_Promotional_Head";
$tr=performQuery($sql);
$trans_act_sl=$tr[0][sl_no];
$cust_feedback=$promotional['cust_feedback'];
$empfd=$promotional['empfd'];

$samp = $promotional['Pro_product'];

$sql = "insert into Trans_Activity_Promotional_Head select '$trans_act_sl','$sfCode',getdate()," . $cCode . ",'$cust_feedback','$empfd','" . $ARCd . "','" . $ARDCd . "','" . $Owndiv . "'";
performQuery($sql);
		for ($j = 0; $j < count($samp); $j++) {
		$sql="insert into Trans_Activity_Promotional_Prod_Detail select '$trans_act_sl','".$samp[$j]["cateid"]."','','".$samp[$j]["product"]."','".$samp[$j]["product_Nm"]."','','".$samp[$j]["rx_qty"]."','".$samp[$j]["sample_qty"]."',''";
 
		performQuery($sql);
		}
}
if($deliveryReturn!=null){
$sql="select isnull(max(Trans_Act_SL),0)+1 sl_no from Trans_Activity_Delivery_Return";
$tr=performQuery($sql);
$trans_act_sl=$tr[0][sl_no];
$del_ret=$deliveryReturn['activity_del_ret'];
$invoiceNo=$deliveryReturn['invoiceNo'];
$ret_feedback=$deliveryReturn['ret_feedback'];
$orderCam=$deliveryReturn['order_cam_copy'];
$orderSig=$deliveryReturn['order_sig_copy'];
$sql = "insert into Trans_Activity_Delivery_Return select '$trans_act_sl','$sfCode',getdate()," . $cCode . ",$del_ret,$invoiceNo,$orderSig,$orderCam,$ret_feedback,'" . $ARCd . "','" . $ARDCd . "','" . $Owndiv . "'";

performQuery($sql);

                    for ($j = 0; $j < count($samp); $j++) {
                    
$sql="insert into Trans_Activity_Delivery_Return_Prod_Detail select '$trans_act_sl','','','".$samp[$j]["product_code"]."','".$samp[$j]["product_Name"]."','".$samp[$j]["Product_Rx_Qty"]."'";
performQuery($sql);
                    }
for ($j = 0; $j < count($Event_Pick_Captures); $j++) {
	                	
						$ev_imgurl=$sfCode."_".$Event_Pick_Captures[$j]["imgurl"];
				 		//$ev_title=$Event_Captures[$j]["title"];
				 		//$ev_remarks=$Event_Captures[$j]["remarks"];
						$sql = "insert into Trans_Activity_Delivery_Return_Challans select '$trans_act_sl','" . $ev_imgurl . "'";
		            	performQuery($sql);
	
					}	
}
if($otAttended!=null){
$sql="select isnull(max(Trans_Act_SL),0)+1 sl_no from Trans_Activity_OT_Attended";
$tr=performQuery($sql);
$trans_act_sl=$tr[0][sl_no];
$remarks=$otAttended['remarks'];
$orderCam=$otAttended['order_cam_copy'];
$orderSig=$otAttended['order_sig_copy'];
$sql = "insert into Trans_Activity_OT_Attended select '$trans_act_sl','$sfCode',getdate()," . $cCode . ",$remarks,'" . $ARCd . "','" . $ARDCd . "','" . $Owndiv . "',$orderSig,$orderCam";

performQuery($sql);

                    for ($j = 0; $j < count($samp); $j++) {
                    
$sql="insert into Trans_Activity_OT_Attended_Prod_Detail select '$trans_act_sl','','','".$samp[$j]["product_code"]."','".$samp[$j]["product_Name"]."','','','',''";
performQuery($sql);
                    }
}
if($ordercreate!=null){
$sql="select isnull(max(Trans_Act_SL),0)+1 sl_no from Trans_Activity_Order_Create";
$tr=performQuery($sql);
$trans_act_sl=$tr[0][sl_no];
$orderNo=$ordercreate['order_no'];
$orderCopy=$ordercreate['order_copy'];
$sql = "insert into Trans_Activity_Order_Create select '$trans_act_sl','$sfCode',getdate()," . $cCode . ",$orderNo,$orderCopy,'" . $ARCd . "','" . $ARDCd . "','" . $Owndiv . "'";

performQuery($sql);

                    for ($j = 0; $j < count($samp); $j++) {
                    
$sql="insert into Trans_Activity_Order_Create_Prod_Detail select '$trans_act_sl','','','".$samp[$j]["product_code"]."','".$samp[$j]["product_Name"]."'";
performQuery($sql);
                    }
}
if($orderfollowup!=null){
$sql="select isnull(max(Trans_Act_SL),0)+1 sl_no from Trans_Activity_Order_Followup";
$tr=performQuery($sql);
$trans_act_sl=$tr[0][sl_no];
$so_no=$orderfollowup['so_no'];
$cust_feedback=$orderfollowup['cus_feedback'];
$emp_feedback=$orderfollowup['emp_feedback'];

$sql = "insert into Trans_Activity_Order_Followup select '$trans_act_sl','$sfCode',getdate()," . $cCode . ",$so_no,$cust_feedback,$emp_feedback,'" . $ARCd . "','" . $ARDCd . "','" . $Owndiv . "'";

performQuery($sql);

                    for ($j = 0; $j < count($samp); $j++) {
                    
$sql="insert into Trans_Activity_Order_Followup_Prod_Detail select '$trans_act_sl','','','".$samp[$j]["product_code"]."','".$samp[$j]["product_Name"]."'";
performQuery($sql);
                    }

                    }
	if($activity_Expense!=null){ 
	$act=$activity_Expense[count($activity_Expense)-1];
	$act_id=$act['activity'];
	$act_name =$act['activity_name'];
	
	for($j=0; $j<count($activity_Expense)-1;$j++){
	$sql="select isnull(max(Trans_Act_SL),0)+1 sl_no from Trans_Activity_Expense_Head";
	$tr=performQuery($sql);
	$trans_act_sl=$tr[0][sl_no];
	$exp_id=$activity_Expense[$j]['id'];
	$exp_name=$activity_Expense[$j]['name'];
	$exp_amount=$activity_Expense[$j]['amount'];
	
	$sql = "insert into Trans_Activity_Expense_Head select '$trans_act_sl','$sfCode',getdate(),'$exp_id','$exp_name','$act_id','$act_name'," . $cCode . ",'" . $ARCd . "','" . $ARDCd . "','" . $Owndiv . "',getdate(),'$exp_amount'";
	performQuery($sql);
	$exp_param=$activity_Expense[$j]['addexp'];

for ($k = 0; $k < count($exp_param); $k++) {
$par_id =$exp_param[$k]['id'];
$par_name=$exp_param[$k]['cap'];
$par_value =$exp_param[$k]['txt'];
$sql = "insert into Trans_Activity_Expense_Detail select '$trans_act_sl','$par_id','$par_name','$par_value'";
	performQuery($sql);
}
		}
	}
...
case "fileAttachment_activity":
 $doctorCode = $_GET['doctor_code'];
        ...
        $ext = $info['extension'];
        $fileName =$doctorCode . "_" . date('d-m-Y') . "_". $file_name . "." . $ext;
        $file_src = '../server/photos/' . $fileName;
...
break;
    case "imgupload":
        ...
        break;
 case "imgupload_activity":
        $sf = $_GET['sf_code'];
        print_r(sf);
        move_uploaded_file($_FILES["imgfile"]["tmp_name"], "../server/photos/" . $sf . "_" . $_FILES["imgfile"]["name"]);
...
$sql = "exec MailInbox_App_New '$sfCode','$Owndiv','$folder','$fldr','$year','$month',''";
//print_r($sql);die;
