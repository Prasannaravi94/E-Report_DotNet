...
function getTourPlan(){
    ...
    $Mnth=(string) $data['Month'];
    $Yr=(string) $data['Year'];
    $query="exec iOS_getTourPlan_new '".$sfCode."','".$Mnth."','".$Yr."'";
    $res=performQuery($query);
...
function SvTourPlan(){
	...
	for ($i = 0; $i < count($TPDatas); $i++) 
    {
		...
		if($TPData["dayno"]!="")
		{
			...
			$TPJWCd=$TPDet["JWCodes"];
			$TPJWNm=$TPDet["JWNames"];
...
global $data;
   ...
   $amc = (string) $data['amc'];
    if (!is_null($amc)) {
      ...
      $sql = "DELETE s  from tbDgDetailing_SlideDetails s inner join tbDigitalDetailing_Head h on s.DDSl_No=h.DDSl_No inner join vwActivity_MSL_Details d on h.Activity_Report_code=d.Trans_SlNo AND h.MSL_code=d.Trans_Detail_Info_Code where Trans_Detail_Slno='" . $amc . "'";
        performQuery($sql);
        $sql = "DELETE h from tbDigitalDetailing_Head h inner join vwActivity_MSL_Details d on h.Activity_Report_code=d.Trans_SlNo AND h.MSL_code=d.Trans_Detail_Info_Code where Trans_Detail_Slno='" . $amc . "'";
        performQuery($sql);
...
function SvDCREntry(){
    ...
    $ARDCd= $data['amc'];
    if($ARDCd!='' && $ARDCd!=null) deleteEntry($ARDCd);
   ...
   if($ARCd==""){
	...
	if($lat =="(null)"){$lat ="";$lng ="";}
    if($data['CusType']=="1"){  
      $query="exec svDCRLstDet_App '".$ARCd."',0,'".$sfCode."',1,'".$CustCode."','".$CustName."','".$VstTime."',0,'".$JWWrk."','".$Prods."','".$ProdsNm."','','','".$GC."','".$GN."','".$GQ."','".$Inps."','".$InpsNm."','','".$data['Remks']."','".$DivCode."',0,'".$ModTime."','".$lat."','".$lng."','".$data['DataSF']."','NA','iOS'";
      //$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
    ...
    if($data['CusType']=="2" || $data['CusType']=="3" ){
    	$query="exec svDCRCSHDet_App '".$ARCd."',0,'".$sfCode."','".$data['CusType']."','".$CustCode."','".$CustName."','".$VstTime."',0,'".$JWWrk."','".$Prods."','".$ProdsNm."','".$Inps."','".$InpsNm."','','".$data['Remks']."','".$DivCode."',0,'".$ModTime."','".$lat."','".$lng."','".$data['DataSF']."','NA','iOS'";
	    		//$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
    	$result["CQry"]=$query;
      	performQuery($query);//performQueryWP($query, $params);
      if($ARDCd==""){
    ...
    if($data['CusType']=="4"){
    	$query="exec svDCRUnlstDet_App '".$ARCd."',0,'".$sfCode."','".$data['CusType']."','".$CustCode."','".$CustName."','".$VstTime."',0,'".$JWWrk."','".$Prods."','".$ProdsNm."','','','".$GC."','".$GN."','".$GQ."','".$Inps."','".$InpsNm."','','".$data['Remks']."','".$DivCode."',0,'".$ModTime."','".$lat."','".$lng."','".$data['DataSF']."','NA','iOS'";
		//$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
    	$result["NQry"]=$query;
      	performQuery($query);//performQueryWP($query, $params);
      if($ARDCd==""){
    ...
    for($Adil=-1;$Adil<count($data["AdCuss"]);$Adil++)
    {
        if($Adil>-1){ 
            ...
            $query="exec svDCRLstDet_App '".$ARCd."',0,'".$sfCode."',1,'".$CustCode."','".$CustName."','".$VstTime."',0,'".$JWWrk."','".$Prods."','".$ProdsNm."','','','','','','','','','".$data['Remks']."','".$DivCode."',0,'".$ModTime."','".$lat."','".$lng."','".$data['DataSF']."','NA','iOS'";
            //$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
            $result["DQry"]=$query;
    	      performQuery($query);//performQueryWP($query, $params);
            if($ARDCd==""){
...
function SvMissDCREntry(){
    ...
    $DivCode = str_replace(",", "", $DivCode);
	  ...
	  $data_dcr = json_encode($_POST['data'],true);
     $data_dcr1 = str_replace("'", "",$data_dcr);
     $sql="insert into tracking_dcr select '$sfCode','$DivCode','$data_dcr',getdate(),'$DCRDt','MEdet'";
	 //echo $sql;
     performQuery($sql);
    ...
    for ($dloop = 0; $dloop < count($CallsData); $dloop++) 
    {
			...
			if($work!=0)
        {
		    ...
		    for ($i = 0; $i < count($ProdsArr); $i++)
		    {
			    ...
			    if($idata['CusType']=="1" || $idata['CusType']=="2"){
					if($ProdsArr[$i]["RxQty"]=="") $ProdsArr[$i]["RxQty"]="0";
				    $Prods=$Prods."$".$ProdsArr[$i]["RxQty"]."$0^0";
				    $ProdsNm=$ProdsNm."$".$ProdsArr[$i]["RxQty"]."$0^0";
			    }
		    ...
		    if($lat =="(null)"){$lat ="";$lng ="";}
		    if($idata['CusType']=="1"){  
			    $query="exec svDCRLstDet_App '".$ARCd."',0,'".$sfCode."',1,'".$CustCode."','".$CustName."','".$VstTime."',0,'".$JWWrk."','".$Prods."','".$ProdsNm."','','','".$GC."','".$GN."','".$GQ."','".$Inps."','".$InpsNm."','','".$idata['Remks']."','".$DivCode."',0,'".$VstTime."','".$lat."','".$lng."','".$idata['DataSF']."','NA','iOS','".$idata['CallType']."'";
			    //$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
			    performQuery($query);//performQueryWP($query, $params);
			    if($ARDCd==""){
				    $query="select Trans_Detail_Slno from vwActivity_MSL_Details where Trans_SlNo='".$ARCd."' and Trans_Detail_Info_Code='".$CustCode."'";
		    ...
		    if($idata['CusType']=="2" || $idata['CusType']=="3" ){
    		    $query="exec svDCRCSHDet_App '".$ARCd."',0,'".$sfCode."','".$idata['CusType']."','".$CustCode."','".$CustName."','".$VstTime."',0,'".$JWWrk."','".$Prods."','".$ProdsNm."','".$Inps."','".$InpsNm."','','".$idata['Remks']."','".$DivCode."',0,'".$VstTime."','".$lat."','".$lng."','".$idata['DataSF']."','NA','iOS'";
	    		    //$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
    		    $result["CQry"]=$query;
      		    performQuery($query);//performQueryWP($query, $params);
			      if($ARDCd==""){
		    ...
		    if($idata['CusType']=="4"){
    		    $query="exec svDCRUnlstDet_App '".$ARCd."',0,'".$sfCode."','".$idata['CusType']."','".$CustCode."','".$CustName."','".$VstTime."',0,'".$JWWrk."','".$Prods."','".$ProdsNm."','','','".$GC."','".$GN."','".$GQ."','".$Inps."','".$InpsNm."','','".$idata['Remks']."','".$DivCode."',0,'".$VstTime."','".$lat."','".$lng."','".$idata['DataSF']."','NA','iOS'";
			    //$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
    		    $result["NQry"]=$query;
      		    performQuery($query);//performQueryWP($query, $params);
			    if($ARDCd==""){
			...
			for($Adil=-1;$Adil<count($idata["AdCuss"]);$Adil++)
    {
        if($Adil>-1){ 
            ...
            $query="exec svDCRLstDet_App '".$ARCd."',0,'".$sfCode."',1,'".$CustCode."','".$CustName."','".$VstTime."',0,'".$JWWrk."','".$Prods."','".$ProdsNm."','','','','','','','','','".$idata['Remks']."','".$DivCode."',0,'".$ModTime."','".$lat."','".$lng."','".$idata['DataSF']."','NA','iOS'";
            //$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
            $result["DQry"]=$query;
    	      performQuery($query);//performQueryWP($query, $params);
            if($ARDCd==""){
   ...
   case "save/tpdaynew";
	...
	//
       ...
       outputJSON(SvMissDCREntry());
        break;
case "save/missentry1";
	//echo 'hi';
	//
	
       outputJSON(SvMissDCREntry1());
        break;
...
$sq="select  Time vstTime,* from vwActivity_Unlst_Detail where Trans_Detail_Slno='".$detno."'";
}
else{
$sq="select Additional_Prod_Code as Product_Code,Additional_Prod_Dtls as Product_Detail,  * from vwActivity_CSH_Detail where Trans_Detail_Slno='".$detno."'";
}
...
$rcpdet="select * from Trans_RCPA_detail where FK_PK_ID='".$b1[$k]["PK_ID"]."'";
