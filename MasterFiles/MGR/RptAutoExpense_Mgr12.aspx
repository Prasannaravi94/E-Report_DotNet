                                ...
                                <Columns>
                                    ...
                                    <asp:TemplateField HeaderText="Place of Work" ItemStyle-HorizontalAlign="Left">
                                     ...
                                     <asp:TemplateField HeaderText="Allowance Type" ItemStyle-HorizontalAlign="Left">
                                        ...
                                        <HeaderStyle BorderStyle="Solid" BorderWidth="1px" BorderColor="#9AA3A9"></HeaderStyle>
                                        <ItemTemplate><asp:Label ID="lblCat" runat="server" Text='<%# Bind("Type_Code") %>'></asp:Label><asp:DropDownList  onchange="onAllowTypeChange(this,'false','')" class="from" ID="AllowType" Visible="false" runat="server" AutoPostBack="false" SkinID="ddlRequired">
...
function onFareChange($e) {
    $R = $e.parentNode.parentNode;
    //alert($R);
    var oldFare = parseFloat($R.cells[7].children[1].value.replace(/,/g, ''));
    var oldRowTot = parseFloat($R.cells[8].children[0].innerHTML.replace(/,/g, ''));
    var $tot = $R.parentNode.children[$R.parentNode.children.length - 1].cells[8].children[0];
    var oldTot = parseFloat($tot.innerHTML.replace(/,/g, ''));
    var $grndFareTot = $R.parentNode.children[$R.parentNode.children.length - 1].cells[7].children[0];
    var oldGrndFareTot = parseFloat($grndFareTot.innerHTML.replace(/,/g, ''));
    // alert(oldRowTot + " :: " + oldTot);
    var newFare = parseFloat($R.cells[7].children[0].value.replace(/,/g, ''));
    if (isNaN(oldFare)) oldFare = 0;
    if (isNaN(newFare)) newFare = 0;
    //alert(newFare + " :: " + oldFare);

    $R.cells[7].children[1].value = newFare;
    $R.cells[8].children[0].innerHTML = oldRowTot - oldFare + newFare;
    $R.cells[8].children[1].value = oldRowTot - oldFare + newFare;
