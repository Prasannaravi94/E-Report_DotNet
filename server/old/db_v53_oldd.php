	  ...
	  for ($Ri = 0; $Ri < count($RCPADatas); $Ri++) 
    {
          ...
          if($ARDCd!="")
          {
              ...
              if(count($arr[0])>0){
                  ...
                  $lng=$arr[0]["long"];
                  $DataSF=$arr[0]["DataSF"];
                  $query="exec svDCRCSHDet_App '".$ARCd."',0,'".$sfCode."','2','".$ChmIds."','".$ChmNms."','".$VstTime."',0,'".$JWWrk."','','','','','','','".$DivCode."',0,'".$VstTime."','".$lat."','".$lng."','".$DataSF."','NA','App'";
...
//echo $sql;
        ...
        case "LeaveReject":
            $leaveid = $_GET['leaveid'];
            $RSF = $_GET['sfCode'];
			$sql = "exec iOS_svLeaveAppRej  " . $leaveid  . ",1,'','".$RSF."','Apps'";
			performQuery($sql);
            break;
        case "LeaveForm":
            $name = $_GET['sf_name'];
			$sql = "select Leave_SName from mas_Leave_Type where Leave_code=" . $vals['Leave_Type'] ."";		
            $tRw = performQuery($sql);
            $lvType = $tRw[0]['Leave_SName'];
			
			$sql = "exec iOS_svLeaveApp '$sfCode','" . $vals['From_Date'] . "'," . $vals['To_Date'] . "," . $vals['No_of_Days'] . ",'" . $lvType. "'," . $vals['Reason'] . "," . $vals['address'] . ",'Apps'";
			performQuery($sql);
...
$divCode = $_GET['divisionCode'];
	...
	case "callCount":
			...
			$divisionCode = explode(",", $divCode);
			$MSL = $_GET['Msl_No'];		
			//$sql = "SELECT COUNT(Trans_Detail_Info_Code) as Visit_tl FROM vwActivity_MSL_Details WHERE sf_code='$sfCode' and Trans_Detail_Info_Code='$MSL' and year(time) = '$year' and month(time) ='$month' ";
			$sql = "SELECT 0 as Visit_tl";
//echo $sql;		
