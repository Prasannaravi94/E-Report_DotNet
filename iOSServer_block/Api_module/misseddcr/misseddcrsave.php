...
function misseddcrsave() {
	...
	$idata=$CallsData[0];
		...
		{ 
			...
			$sql="insert into tracking_edet_dcr select '" .$data['sfcode']. "','" . $DivCode[0] . "','".$data."',getdate(),'$DCRDt','Edet'";
			performQuery($sql);
			...
			if($work!=0)
			{
				...
				if($data['CusType']=="2" || $data['CusType']=="3" ){
					...
					$details=cshdcr($ARCd,$Prods,$ProdsNm,$Inps,$InpsNm,$JWWrk,$JWWrkNm,$GC,$GN,$GQ,$StockNm,$data,$DCRDt);
					$loadcsh=1;
				}
				if($data['CusType']=="4"){
				//	if($loadunlisted==0)
