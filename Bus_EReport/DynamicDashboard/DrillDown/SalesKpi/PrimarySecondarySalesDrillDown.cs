...
namespace Bus_EReport.DynamicDashboard.DrillDown.SalesKpi
{
    ...
    public class PrimarySecondarySalesDrillDown : DrilldownBase
    {
        ...
        public PrimarySecondarySalesDrillDown()
        {
            ...
            Columns = new Dictionary<string, DrillDownColumn>
            {
                ...
                {"SecondarySaleQuantity", new DrillDownColumn {Label = "Secondary Sale Quantity", Select = "Trans_SS_Entry_Detail_value.Sec_Sale_Qty as SecondarySaleQuantity"}},
                {"SecondarySaleValue", new DrillDownColumn {Label = "Secondary Sale Value", Select = "CAST(Trans_SS_Entry_Detail_value.Sec_Sale_Qty AS float) * Trans_SS_Entry_Detail." + SecsalesValueColumn+" as SecondarySaleValue"}},
                {"PrimarySaleQuantity", new DrillDownColumn {Label = "Primary Sale Quantity", Select = "0 as PrimarySaleQuantity"}},
                {"PrimarySaleValue", new DrillDownColumn {Label = "Primary Sale Value", Select = "0 as PrimarySaleValue"}},
            };
        ...
        {
            FileName = "Pirmary vs Secondary Sales";
            PreferenceName = "PrimarySecondarySalesDrillDown";
        ...
        public String GetPrimarySalesQuery()
        {
            ...
            strQry +="FORMAT(Primary_Bill.Inv_Date, 'MMM yyyy') as Month";
            if (VisibleColumns.Contains("Stockist"))
            {
                strQry += ",Stockists.Stockist_Name AS Stockist";
            }
            if (VisibleColumns.Contains("HQName"))
            {
                strQry += ",Stockists.Approved_By AS HQName";
            }
            if (VisibleColumns.Contains("StateName"))
            {
                strQry += ",Mas_State.StateName AS StateName";
            }
            if (VisibleColumns.Contains("ProductName"))
            {
                strQry += ",Mas_Product_Detail.Product_Detail_Name AS ProductName";
            }
            if (VisibleColumns.Contains("SecondarySaleQuantity"))
            {
                strQry += ",0 AS SecondarySaleQuantity";
            }
            if (VisibleColumns.Contains("SecondarySaleValue"))
            {
                strQry += ",0 AS SecondarySaleValue";
            }
            if (VisibleColumns.Contains("PrimarySaleQuantity"))
            {
                strQry += ",Primary_bill.Sale_Qty AS PrimarySaleQuantity";
            }
            if (VisibleColumns.Contains("PrimarySaleValue"))
            {
                strQry += ",Primary_bill.Sale_Value AS PrimarySaleValue";
            }
            if (VisibleColumns.Contains("BrandName"))
            {
                strQry += ",Mas_Product_Brand.Product_Brd_Name AS BrandName";
            }
            if (VisibleColumns.Contains("CategoryName"))
            {
                strQry += ",Mas_Product_Category.Product_Cat_Name AS CategoryName";
            }
            if (VisibleColumns.Contains("GroupName"))
            {
                strQry += ",Mas_Product_Group.Product_Grp_Name AS GroupName";
            }
            strQry += " FROM " +
                "Primary_Bill " +
                "INNER JOIN " +
        ...
        public string getQuery(bool count =false)
        {
            ...
            strQry += " ) ";
            strQry += ", SecSales AS ("+GetSecondarySalesQuery()+")";
            strQry += ", MonthWiseSecSales As (";
            strQry += "SELECT ";
            ...
            if (VisibleColumns.Contains("SecondarySaleQuantity"))
            {
                strQry += ",SUM(SecondarySaleQuantity) AS SecondarySaleQuantity";
            }
            if (VisibleColumns.Contains("SecondarySaleValue"))
            {
                strQry += ",SUM(SecondarySaleValue) AS SecondarySaleValue";
            }
            if (VisibleColumns.Contains("PrimarySaleQuantity"))
            {
                strQry += ",0 AS PrimarySaleQuantity";
            }
            if (VisibleColumns.Contains("PrimarySaleValue"))
            {
                strQry += ",0 AS PrimarySaleValue";
            }
            if (VisibleColumns.Contains("BrandName"))
            {
                strQry += ",BrandName";
            ...
            strQry += ")";
            strQry += ", Bills AS ("+GetPrimarySalesQuery()+")";
            strQry += ", MonthWiseBills As (";
            strQry += "SELECT ";
            ...
            if (VisibleColumns.Contains("PrimarySaleQuantity"))
            {
                strQry += ",SUM(PrimarySaleQuantity) AS PrimarySaleQuantity";
            }
            if (VisibleColumns.Contains("PrimarySaleValue"))
            {
                strQry += ",SUM(PrimarySaleValue) AS PrimarySaleValue";
            }
            if (VisibleColumns.Contains("BrandName"))
            {
                strQry += ",BrandName";
            ...
            strQry += ",Result as (Select ";
                strQry += " COALESCE(MonthWiseSecSales.Month, MonthWiseBills.Month) AS Month ";
            if (VisibleColumns.Contains("Stockist"))
            {
                strQry += " ,COALESCE(MonthWiseSecSales.Stockist, MonthWiseBills.Stockist) AS Stockist ";
            }
            if (VisibleColumns.Contains("HQName"))
            {
                strQry += " ,COALESCE(MonthWiseSecSales.HQName, MonthWiseBills.HQName) AS HQName ";
            }
            
            if (VisibleColumns.Contains("StateName"))
            {
                strQry += " ,COALESCE(MonthWiseSecSales.StateName, MonthWiseBills.StateName) AS StateName ";
            }
            if (VisibleColumns.Contains("ProductName"))
            {
                strQry += " ,COALESCE(MonthWiseSecSales.ProductName, MonthWiseBills.ProductName) AS ProductName ";
            }
            if (VisibleColumns.Contains("BrandName"))
            {
                strQry += " ,COALESCE(MonthWiseSecSales.BrandName, MonthWiseBills.BrandName) AS BrandName ";
            }
            if (VisibleColumns.Contains("CategoryName"))
            {
                strQry += " ,COALESCE(MonthWiseSecSales.CategoryName, MonthWiseBills.CategoryName) AS CategoryName ";
            }
            if (VisibleColumns.Contains("GroupName"))
            {
                strQry += " ,COALESCE(MonthWiseSecSales.GroupName, MonthWiseBills.GroupName) AS GroupName ";
            }
            if (VisibleColumns.Contains("SecondarySaleQuantity"))
            {
                strQry += ",MonthWiseSecSales.SecondarySaleQuantity";
            }
            if (VisibleColumns.Contains("SecondarySaleValue"))
            {
                strQry += ",MonthWiseSecSales.SecondarySaleValue";
            }
            if (VisibleColumns.Contains("PrimarySaleQuantity"))
            {
                strQry += ",MonthWiseBills.PrimarySaleQuantity";
            }
            if (VisibleColumns.Contains("PrimarySaleValue"))
            {
                strQry += ",MonthWiseBills.PrimarySaleValue";
            }
            strQry += " FROM  MonthWiseBills ";
            strQry += "FULL JOIN  MonthWiseSecSales ON  1=1 ";
            if (VisibleColumns.Contains("Stockist"))
            {
                strQry += " AND MonthWiseBills.Stockist=MonthWiseSecSales.Stockist ";
            }
            if (VisibleColumns.Contains("HQName"))
            {
                strQry += " AND MonthWiseBills.HQName=MonthWiseSecSales.HQName ";
            }
            if (VisibleColumns.Contains("Month"))
            {
                strQry += " AND MonthWiseBills.Month=MonthWiseSecSales.Month ";
            }
            if (VisibleColumns.Contains("StateName"))
            {
                strQry += " AND MonthWiseBills.StateName=MonthWiseSecSales.StateName ";
            }
            if (VisibleColumns.Contains("ProductName"))
            {
                strQry += " AND MonthWiseBills.ProductName=MonthWiseSecSales.ProductName ";
            }
            if (VisibleColumns.Contains("BrandName"))
            {
                strQry += " AND MonthWiseBills.BrandName=MonthWiseSecSales.BrandName ";
            }
            if (VisibleColumns.Contains("CategoryName"))
            {
                strQry += " AND MonthWiseBills.CategoryName=MonthWiseSecSales.CategoryName ";
            }
            if (VisibleColumns.Contains("GroupName"))
            {
                strQry += " AND MonthWiseBills.GroupName=MonthWiseSecSales.GroupName ";
            }

            strQry += ")";
            if (count)
            {
                strQry += "Select count(*) as totalRecords from Result ";
            }
            else
            {
                strQry += "Select * from Result ";
            }
            
            if (!IsExport && !count)
            {
                strQry += " ORDER BY Result.Month  OFFSET " + (StartRow - 1) + " ROWS FETCH NEXT " + (EndRow - StartRow + 1) + " ROWS ONLY ";
            }
