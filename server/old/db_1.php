...
if($AppDeviceRegId==null){
  $respon = array();
        $respon['success'] = false;
 $respon['type'] = 3;
        $respon['msg'] = "Old App Changed,.Kindly Use Link is www.sansfa.info/app.html";
...
$sfCode=$_GET['rSF'];
 ...
 case "get/track":
    	...
    	$Dt = date('Y-m-d H:i:s', strtotime($_GET['Dt']));
		/*$query = "select * from ((select replace(convert(varchar,Time,9),':000',' ') DtTime,lati lat,Long lng,Trans_Detail_Name,GeoAddrs,Time DtTm,0 ordfld from vwActivity_MSL_Details where  cast(convert(varchar,Time,101) as datetime)=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and Time>'$Dt'  and sf_code='$SF')union(select replace(convert(varchar,DtTm,9),':000',' ') DtTime,lat,Lon lng,'','',DtTm,1 from tbTrackLoction where SF_Code='$SF'  and cast(convert(varchar,DtTm,101) as datetime)=cast(convert(varchar(10),cast('$Dt' as datetime),101) as datetime) and DtTm>'$Dt' )) as t order by DtTm,ordfld";*/
$query = "select * from (select replace(convert(varchar,DtTm,9),':000',' ') DtTime,lat,Lon lng,'' Trans_Detail_Name,'' GeoAddrs,DtTm,1 ordfld  from tbTrackLoction where SF_Code='$SF'  and cast(convert(varchar,DtTm,101) as datetime)=cast(convert(varchar(10),cast('$Dt' as datetime),101) as datetime) and DtTm>'$Dt' ) as t order by DtTm,ordfld";
    	$result = performQuery($query);
		outputJSON($result);
        break;
case "get/mytp":
		$SF=$_GET['SF_Code'];
    	$Dt = substr($_GET['Dt'],0,19);
		$query = "select Wtype,ClstrName from vwMyDayPlanAll v inner join Mas_Salesforce S on S.Sf_Code=v.SF_Code inner join vwMas_WorkType_all w on w.type_code=worktype and SFTyp=sf_type  where v.SF_Code='$SF' and Pln_date='$Dt'";
        $result = performQuery($query);
		outputJSON($result);
        break;
  case "get/dcrtrack":
		  $SF=$_GET['SF_Code'];
    	$Dt = substr($_GET['Dt'],0,20);
		  $Mode=$_GET['Mode'];
      
      if($Mode=='D')
      {
		   //$query = "select * from ((select replace(convert(varchar,Time,9),':000',' ') DtTime,lati lat,Long lng,Trans_Detail_Name,GeoAddrs,Time DtTm,0 ordfld from vwActivity_MSL_Details where  cast(convert(varchar,Time,101) as datetime)=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime)  and sf_code='$SF')union(select replace(convert(varchar,DtTm,9),':000',' ') DtTime,lat,Lon lng,'','',DtTm,1 from tbTrackLoction where SF_Code='$SF'  and cast(convert(varchar,DtTm,101) as datetime)=cast(convert(varchar(10),cast('$Dt' as datetime),101) as datetime) and DtTm>'$Dt' )) as t order by DtTm,ordfld";
      $query = "select replace(convert(varchar,Time,9),':000',' ') DtTime,lati lat,Long lng,Trans_Detail_Name,GeoAddrs,Time DtTm,0 ordfld from vwActivity_MSL_Details where  DCRDt=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and sf_code='$SF' and Time>'$Dt'  order by DtTm,ordfld";

      $result = performQuery($query);
      }
      else if($Mode=='C')
      {
      $query = "select replace(convert(varchar,ATime,9),':000',' ') DtTime,lati lat,Long lng,Trans_Detail_Name,GeoAddrs,ATime DtTm,0 ordfld from vwActivity_CSH_Detail where  DCRDt=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and sf_code='$SF' and Trans_Detail_Info_Type=2  order by DtTm,ordfld";
      $result = performQuery($query);
      
      }
      else if($Mode=='S')
      {
      $query = "select replace(convert(varchar,ATime,9),':000',' ') DtTime,lati lat,Long lng,Trans_Detail_Name,GeoAddrs,ATime DtTm,0 ordfld from vwActivity_CSH_Detail where  DCRDt=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and sf_code='$SF' and Trans_Detail_Info_Type=3 order by DtTm,ordfld";
      $result = performQuery($query);
      
      }
      else if($Mode=='ULD')
      {
      $query = "select replace(convert(varchar,ETime,9),':000',' ') DtTime,lati lat,Long lng,Trans_Detail_Name,GeoAddrs,ETime DtTm,0 ordfld from vwActivity_Unlst_Detail where  DCRDt=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and sf_code='$SF'  order by DtTm,ordfld";
      $result = performQuery($query);
      } 
		 outputJSON($result);
     break;      
