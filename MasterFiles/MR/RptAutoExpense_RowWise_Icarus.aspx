<%@ Page Language="C#" AutoEventWireup="true" CodeFile="RptAutoExpense_RowWise_Icarus.aspx.cs" Inherits="MasterFiles_MR_RptAutoExpense" %>
                                ...
                                <Columns>
                                    ...
                                    <asp:TemplateField HeaderText="Place of Work" ItemStyle-HorizontalAlign="Left">
                                     ...
                                     <asp:TemplateField HeaderText="Fare</br>(in Rs/-)" ItemStyle-HorizontalAlign="Left">
                                        ...
                                        <HeaderStyle BorderStyle="Solid" BorderWidth="1px" BorderColor="#9AA3A9"></HeaderStyle>
                                        <ItemTemplate><asp:TextBox ID="txtFare" onkeyup="onFareChange(this);CheckNumeric(event);" visible="false" width="30" runat="server" Text='<%# Bind("Fare") %>'></asp:TextBox><asp:Label ID="lblFare" runat="server" Text='<%# Bind("Fare") %>'></asp:Label><asp:HiddenField ID="fareHidden" runat="server" Value='<%# Bind("Fare") %>' />
                                        </ItemTemplate>
                                    </asp:TemplateField>
                                     <asp:TemplateField HeaderText="Miscellaneous Expenses ( In Rs )" ItemStyle-HorizontalAlign="Left" >
                                    <ControlStyle></ControlStyle>
                                    <ItemStyle BorderStyle="Solid" BorderWidth="1px" BorderColor="#9AA3A9" HorizontalAlign="center">
                                    </ItemStyle>
                                    <HeaderStyle BorderStyle="Solid" BorderWidth="1px" BorderColor="#9AA3A9"></HeaderStyle>
                                    <ItemTemplate>
                                        <asp:TextBox runat="server" ID="txtaddexp" onkeyup="onaddExpChange(this);" Text='<%# Bind("rw_amount") %>' onkeypress="CheckNumeric(event);" Width="30"></asp:TextBox>
                                        <asp:Label ID="lbladdexp" runat="server" Text='<%# Bind("rw_amount") %>' Visible="false"></asp:Label>
                                        <asp:HiddenField ID="addExpHidden" runat="server" Value='<%# Bind("rw_amount") %>' />
                                    </ItemTemplate>
                                </asp:TemplateField>
                                <asp:TemplateField HeaderText="Remarks ( 50 Char )" ItemStyle-HorizontalAlign="Left">
                                    <ControlStyle></ControlStyle>
                                    <ItemStyle BorderStyle="Solid" BorderWidth="1px" BorderColor="#9AA3A9" HorizontalAlign="center">
                                    </ItemStyle>
                                    <HeaderStyle BorderStyle="Solid" BorderWidth="1px" BorderColor="#9AA3A9"></HeaderStyle>
                                    <ItemTemplate>
                                        <asp:TextBox ID="txtrmks" runat="server" Text='<%# Bind("rw_rmks")%>' onkeypress="AlphaNumeric_NoSpecialChars(event);" Width="150"></asp:TextBox>
...
//        }
        ...
        var oldTot = parseFloat($R1.cells[11].childNodes[0].innerHTML.replace(/,/g, ''));
        if (isNaN(allw)) allw = 0;
        if (isNaN(newFare)) newFare = 0;
        if (isNaN(oldTot)) oldTot = 0;
        var newTot = parseFloat(newFare) + parseFloat(allw);
        $R1.cells[11].childNodes[0].innerHTML = parseFloat(newTot);
        $R1.cells[11].childNodes[1].value = parseFloat(newTot);
        //alert($R1.parentNode.children.length - 1);
        $tot = $R1.parentNode.children[$R1.parentNode.children.length - 1].cells[11].children[0];
    ...
    function onAllowTypeChange($e, flag, type) {
        ...
        $R.cells[11].children[0].innerHTML = oldRowTot - oldAllow + newAllow;
        $R.cells[11].children[1].value = oldRowTot - oldAllow + newAllow;
    ...
    function onFareChange($e) {
        ...
        $R.cells[11].children[0].innerHTML = oldRowTot - oldFare + newFare;
        $R.cells[11].children[1].value = oldRowTot - oldFare + newFare;
