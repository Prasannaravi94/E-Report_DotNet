...
function getLeaveType() {
   ...
   // $query = "select Leave_code,Leave_SName,Leave_Name,Division_Code,Active_Flag,Created_Date from mas_Leave_Type where division_code='".$div_code[0]."'";	
	$query = "select * from mas_Leave_Type where division_code='".$div_code[0]."' and Active_Flag='0'";		
	return performQuery($query);
}
...
function SvMyTodayTP(){
    ...
    $query="select SF_type from Mas_Salesforce_two with (nolock) where Sf_Code='".$sfCode."' and Division_Code='".$DivCode[0].","."'";
    $ExisArr = performQuery($query);
	$SFTy=$ExisArr[0]["SF_type"];
	// if($WT=="0" || $WT=="" || $WT==" ")
    // {
        $query="select type_code from vwMas_WorkType_all where Division_Code='".$DivCode[0]."' and SFTyp='".$SFTy."' and Wtype='".$WTNM."' ";
	  ...
	  if ($ExisArr[0]["Cnt"]>0){
        ...
        performQuery(str_replace("DCRMain_Trans", "DCRMain_Temp", $query));
			...
			{ 
				...
				$query="exec svDCRMain_App '" . $sfCode . "','" . date('Y-m-d 00:00:00.000', strtotime($TPDt)) . "','" . $WT . "','" . $PlnCd . "','" . $DivCode[0] . "','" . $Rem . "','','".$mod."'";
				performQuery($query);
			}
...
function SvDCREntry(){
   ...
   //  $data_dcr1 = str_replace("'", "",$data_dcr);
     $sql="insert into tracking_dcr_edet select '$sfCode','$DivCode','$data_dcr',getdate(),'$DCRDt','Edet'";
     performQuery($sql);
	
	 $ARDCd= $data['amc'];
    if($ARDCd!='') deleteEntry($ARDCd);
    ...
    // {
		$query="select SF_type from Mas_Salesforce_two with (nolock) where Sf_Code='".$sfCode."' and Division_Code='".$DivCode.","."'";
   ...
   // }
    ...
    $query="exec svDCRMain_App '".$sfCode."','".$DCRDt."','".$data['WT']."','".$data['Pl']."','".$DivCode."','".$data['Rem']."','','".$mod."'";
   ...
   // performQuery($query);
	...
	if($lat =="(null)"){$lat ="";$lng ="";}
    if($data['CusType']=="1"){  
      $query="exec svDCRLstDet_App '".$ARCd."',0,'".$sfCode."',1,'".$CustCode."','".$CustName."','".$VstTime."','".$DCSUPOB."','".$JWWrk."','".$Prods."','".$ProdsNm."','','','".$GC."','".$GN."','".$GQ."','".$Inps."','".$InpsNm."','','".$data['Remks']."','".$DivCode."','".$Drcallfeedbackcode."','".$ModTime."','".$lat."','".$lng."','".$data['DataSF']."','NA','".$mod."'";
      //$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
    ...
    if($data['CusType']=="2" || $data['CusType']=="3" ){
    	$query="exec svDCRCSHDet_App '".$ARCd."',0,'".$sfCode."','".$data['CusType']."','".$CustCode."','".$CustName."','".$VstTime."','".$DCSUPOB."','".$JWWrk."','".$Prods."','".$ProdsNm."','".$Inps."','".$InpsNm."','','".$data['Remks']."','".$DivCode."','".$Drcallfeedbackcode."','".$ModTime."','".$lat."','".$lng."','".$data['DataSF']."','NA','".$mod."'";
	    		//$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
    	$result["CQry"]=$query;
      	performQuery($query);//performQueryWP($query, $params);
      if($ARDCd==""){
    ...
    if($data['CusType']=="4"){
    	$query="exec svDCRUnlstDet_App '".$ARCd."',0,'".$sfCode."','".$data['CusType']."','".$CustCode."','".$CustName."','".$VstTime."','".$DCSUPOB."','".$JWWrk."','".$Prods."','".$ProdsNm."','','','".$GC."','".$GN."','".$GQ."','".$Inps."','".$InpsNm."','','".$data['Remks']."','".$DivCode."','".$Drcallfeedbackcode."','".$ModTime."','".$lat."','".$lng."','".$data['DataSF']."','NA','".$mod."'";
		//$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
    	$result["NQry"]=$query;
      	performQuery($query);//performQueryWP($query, $params);
      if($ARDCd==""){
    ...
    for($Adil=-1;$Adil<count($data["AdCuss"]);$Adil++)
    {
        if($Adil>-1){ 
            ...
            $query="exec svDCRLstDet_App '".$ARCd."',0,'".$sfCode."',1,'".$CustCode."','".$CustName."','".$VstTime."','".$DCSUPOB."','".$JWWrk."','".$Prods."','".$ProdsNm."','','','','','','','','','".$data['Remks']."','".$DivCode."','".$Drcallfeedbackcode."','".$ModTime."','".$lat."','".$lng."','".$data['DataSF']."','NA','".$mod."'";
            //$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
            $result["DQry"]=$query;
    	      performQuery($query);//performQueryWP($query, $params);
            if($ARDCd==""){
...
function SvMissDCREntry(){
    ...
    for ($dloop = 0; $dloop < count($CallsData); $dloop++) 
    {
			...
			$sql="insert into tracking_dcr_edet select '$sfCode','$DivCode','$idata',getdate(),'$DCRDt','Edet'";
			performQuery($sql);
		    $ARCd = "";
		    $ARDCd = ""; /*(strlen($_GET['amc']) == 0) ? "0" : $_GET['amc'];*/
			...
			if($work!=0)
        {
		    ...
		    $query="exec svDCRMain_App '".$sfCode."','".$DCRDt."','".$idata['WT']."','".$idata['Pl']."','".$DivCode."','".$idata['Rem']."','','".$mod."'";
		    $params = array(array($ARCd, SQLSRV_PARAM_OUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
		    $result["HQry"]=$query;
		    // performQueryWP($query, $params);
		    performQuery($query);
		    ...
		    if($lat =="(null)"){$lat ="";$lng ="";}
		    if($idata['CusType']=="1"){  
			    $query="exec svDCRLstDet_App '".$ARCd."',0,'".$sfCode."',1,'".$CustCode."','".$CustName."','".$VstTime."','".$DCSUPOB."','".$JWWrk."','".$Prods."','".$ProdsNm."','','','".$GC."','".$GN."','".$GQ."','".$Inps."','".$InpsNm."','','".$idata['Remks']."','".$DivCode."','".$Drcallfeedbackcode."','".$VstTime."','".$lat."','".$lng."','".$idata['DataSF']."','NA','".$mod."','".$idata['CallType']."'";
			    //$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
			    performQuery($query);//performQueryWP($query, $params);
			    if($ARDCd==""){
				    $query="select Trans_Detail_Slno from vwActivity_MSL_Details where Trans_SlNo='".$ARCd."' and Trans_Detail_Info_Code='".$CustCode."'";
		    ...
		    if($idata['CusType']=="2" || $idata['CusType']=="3" ){
    		    $query="exec svDCRCSHDet_App '".$ARCd."',0,'".$sfCode."','".$idata['CusType']."','".$CustCode."','".$CustName."','".$VstTime."','".$DCSUPOB."','".$JWWrk."','".$Prods."','".$ProdsNm."','".$Inps."','".$InpsNm."','','".$idata['Remks']."','".$DivCode."','".$Drcallfeedbackcode."','".$VstTime."','".$lat."','".$lng."','".$idata['DataSF']."','NA','".$mod."'";
	    		    //$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
    		    $result["CQry"]=$query;
      		    performQuery($query);//performQueryWP($query, $params);
			      if($ARDCd==""){
		    ...
		    if($idata['CusType']=="4"){
    		    $query="exec svDCRUnlstDet_App '".$ARCd."',0,'".$sfCode."','".$idata['CusType']."','".$CustCode."','".$CustName."','".$VstTime."','".$DCSUPOB."','".$JWWrk."','".$Prods."','".$ProdsNm."','','','".$GC."','".$GN."','".$GQ."','".$Inps."','".$InpsNm."','','".$idata['Remks']."','".$DivCode."','".$Drcallfeedbackcode."','".$VstTime."','".$lat."','".$lng."','".$idata['DataSF']."','NA','".$mod."'";
			    //$params = array(array($ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING(SQLSRV_ENC_CHAR), SQLSRV_SQLTYPE_VARCHAR(50)));
    		    $result["NQry"]=$query;
      		    performQuery($query);//performQueryWP($query, $params);
			    if($ARDCd==""){
...
$arrpob[]=array(substr($def[$k],$poss+1));
}
}
...
$rcpdet="select b.FK_PK_ID,b.CompCode,b.CompName,b.CompPCode,b.CompPName,b.CPQty,b.CPRate,b.CPValue,a.ChmName as Chemname,a.ChmCode as Chemcode from trans_rcpa_head a inner join trans_rcpa_detail b on a.PK_ID =b.fk_pk_id where sf_code='".$sf."' and pk_id='".$b1[$k]["PK_ID"]."'";
...
case "get/vwdcr";
	  ...
	  $sql = "select d.Plan_Name,d.Trans_SlNo,d.Sf_Code,d.FieldWork_Indicator,d.WorkType_Name,d.Sf_Name,convert(varchar,Activity_Date,103) Activity_Date,s.Reporting_To_SF from DCRMain_Temp d
            inner join Mas_Salesforce_two with (nolock) s on d.Sf_Code=s.Sf_Code
...
//MVD_Dashboard_App '2,','mgr0088','1','2022','02-01-2021'			
		...
		case "get/hierarchy":
			...
			$divC = $_GET['divisionCode'];
        	$sfCode = $_GET['sfCode'];			
			$query = "select  division_code,Sf_Code,Sf_Name,sf_type from Mas_Salesforce_two with (nolock) where Reporting_To_SF='".$sfCode."' and division_code='".$divC."'";
			$res =  performQuery($query);
			if(count($res) ==0)
				$query = "select  division_code,Sf_Code,Sf_Name,sf_type from Mas_Salesforce_two with (nolock) where sf_code='".$sfCode."' and division_code='".$divC."' ";
			$res =  performQuery($query);
			outputJSON($res);
		break;
	...
	case "get/primary_sale_groupwise":
		...
		case "get/visit_control":
			global $data;
			$SFCode = (string) $data['SF'];
			$query = "select CustCode,CustType, convert(varchar, Vst_Date, 23)Dcr_dt,month(Vst_Date) Mnth,year(Vst_Date) Yr,CustName,isnull(SDP,'')town_code,isnull(SDP_Name,'')town_name,1 Dcr_flag from tbVisit_Details where SF_Code='$SFCode' and (CustType=1 OR CustType = 0) and  cast(CONVERT(varchar,Vst_Date,101)as datetime) >= DATEADD(mm, DATEDIFF(mm, 0, GETDATE()) - 1, 0) order by Vst_Date";
			//echo $query;
			$results = performQuery($query);
			outputJSON($results);
			break;	
