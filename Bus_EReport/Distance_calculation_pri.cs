...
namespace Bus_EReport
{
    public class Distance_calculation
    {
        ...
        public DataTable getAllowFare(string month, string year)
        {
            ...
            strQry = "select sum(expense_allowance)allw,SUM(Expense_Fare) fare,isnull(SUM(rw_amount),0) rw_amount,sndhqfl Status,SF_Code,submission_date,H.sl_no from Trans_Expense_Head H inner join Trans_Expense_Detail D on H.Sl_No=D.sl_no where Expense_Month=" + month + " and Expense_Year=" + year + " group by SF_Code,sndhqfl,submission_date,H.sl_no";
            try
            {
...
"where H.Sf_Code='" + sf_code + "' and MONTH(Activity_Date)=" + month + " and YEAR(Activity_Date)=" + year + ")as DD";
        ...
        public DataTable getFieldForce(string divcode, string sfcode)
        {
            ...
            DataTable dsSubDiv = null;
            strQry = "SELECT sf_code,sf_name,sf_hq,sf_type,sf_emp_id as Employee_Id,sf_designation_short_name,sf_desgn " +
  ...
  "and b.Leave_Active_Flag=0 and MONTH(a.Activity_Date)='" + month + "' and YEAR(a.activity_date)='" + year + "'";
        ...
        private int insertHeadRecord(Dictionary<String, String> values, int iReturn, DB_EReporting db)
        {
            strQry = "INSERT INTO Trans_Expense_Head " +
                "(Sl_No,SF_Code,Expense_Month,Expense_Year,Division_Code,sndhqfl,submission_date,Expense_Period,File_Path,frmTovalues)" +
                "VALUES ((select isnull(max(Sl_No)+1,1) id from Trans_Expense_Head),'" +
                values["sf_code"] + "'," + values["month"] + "," + values["year"] + "," + values["div_code"] + "," + values["flag"] + ",getdate()," + values["flag"] + ",'" + values["File_Path"] + "','" + values["frmTovalues"] + "')";
            iReturn = db.ExecQry(strQry);
            return iReturn;
        }
 ...
 "where D.sf_code='" + sf_code + "' and Territory_Cat='4' and Territory_name in(" + nextPlace + ") and Territory_name in(" + currPlace + "))as d";
        ...
        public double getOSDistanceBySP_bak(string callType, string places, string prevPlace, string sfcode, out double distance, string np, out int typ, out string frmToPls, out double rangeMult)
        {
        ...
        public DataSet getAllowWorkTypeMetro(string divcode)
        {
            DB_EReporting db_ER = new DB_EReporting();
        ...
        public DataSet NonMetro(string divcode)
        {
            DB_EReporting db_ER = new DB_EReporting();
        ...
        public DataTable getVacStsMnth(string month, string year)
        {
            ...
            strQry = "select dt,S.SF_code,S.Employee_ID from(select max(convert(datetime,adate1,103)) dt,sf_Code from trans_expense_detail D inner join trans_expense_head H on D.sl_no=H.sl_no where Expense_Month=" + month + " and Expense_Year=" + year + " and resigned_flag is null group by sf_code)as d inner join salesforce_vacant S on d.sf_code=S.sf_Code and dt<=dcr_end_date";
            try
            {
                dsSubDiv = db_ER.Exec_DataTable(strQry);
            }
        ...
        public DataTable getAllowFareResigned(string month, string year)
        {
            ...
            DataTable dsSubDiv = null;
            strQry = "select sum(expense_allowance)allw,SUM(Expense_Fare) fare,sndhqfl Status,H.SF_Code,submission_date,S.Employee_Id,H.sl_no from Trans_Expense_Head H inner join Trans_Expense_Detail D on H.Sl_No=D.sl_no inner join salesforce_vacant S on S.sf_code=H.sf_code and S.DCR_end_date=H.DCR_end_date where Expense_Month=" + month + " and Expense_Year=" + year + " and resigned_flag in(1,2) group by H.SF_Code,sndhqfl,submission_date,S.Employee_Id,H.sl_no";
            try
            {
                dsSubDiv = db_ER.Exec_DataTable(strQry);
            }
        ...
        public DataTable getmisResigned(string month, string year)
        {
            ...
            DataTable dsSubDiv = null;
            strQry = "select H.SF_Code,isnull(SUM(Amt),0)mis_Amt,S.Employee_Id from Exp_Others O inner join Trans_Expense_Head H on O.sl_No=H.Sl_No inner join salesforce_vacant S on S.sf_code=H.sf_code and S.dcr_end_Date=H.dcr_end_date where  Expense_Month=" + month + " and Expense_Year=" + year + " and resigned_flag in(1,2) group by H.SF_Code,S.Employee_Id";
            try
            {
                dsSubDiv = db_ER.Exec_DataTable(strQry);
            }
        ...
        public DataTable getOtherExpDetailsResigned(string month, string year)
        {
            ...
            DataTable dsSubDiv = null;
            strQry = "select D.Expense_Parameter_Code,Expense_Parameter_Name,H.SF_Code,Amt,S.Employee_Id from exp_fixed D inner join trans_expense_head H on D.sl_no=H.sl_no inner join salesforce_vacant S on S.sf_Code=H.sf_Code and S.dcr_End_Date=H.dcr_End_date inner join Fixed_Variable_Expense_Setup F on F.expense_parameter_code=D.Expense_Parameter_Code where resigned_flag in(1,2) and Expense_Month=" + month + " and Expense_Year=" + year;
            try
            {
                dsSubDiv = db_ER.Exec_DataTable(strQry);
            }
        ...
        public DataTable getApproveamntResigned(string month, string year)
        {
            ...
            strQry = "select distinct grand_total,H.sf_code,typ,amt from Exp_AccInf E " +
              "inner join Trans_Expense_Head H on H.sl_no=E.sl_No inner join salesforce_vacant S on S.sf_Code=H.sf_Code and S.dcr_End_Date=H.dcr_End_date WHERE resigned_flag=1 and H.Expense_Month=" + month + " and H.Expense_Year=" + year + " ";
        ...
        public DataTable getSavedRecordResigned(string month, string year, string sf_code, string slno)
        {
            ...
            {
                DB_EReporting db = new DB_EReporting();
                strQry = "SELECT catTemp,Expense_Date Adate, adate1, Expense_Day theDayName ,Expense_wtype Worktype_Name_B,Place_of_Work Territory_Name,Expense_All_Type Territory_Cat,Expense_Allowance Allowance,Expense_Distance Distance,Expense_Fare Fare,Expense_Total Total,Division_Code,from_place,to_place,exp_remarks from Trans_Expense_detail " +
                          "where sl_no in(SELECT sl_No FROM Trans_Expense_Head WHERE SF_Code='" + sf_code + "' and Expense_Month=" + month + " and Expense_Year=" + year + " and sl_no='" + slno + "') order by adate1 asc";
                data = db.Exec_DataTable(strQry);
            }
            catch (Exception ex)
            {
