...
public partial class MGR_Dashboard : System.Web.UI.Page
{
    ...
    public static List<values> getChartVal(List<string> lst_Input_Mn_Yr)
    {
        ...
        using (SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["Ereportcon"].ConnectionString))
        {
            ...
            cmd.CommandText = "Manager_Call_avg";
            ...
            if (dr.HasRows)
            {
                while (dr.Read())
                {
                    ...
                    cpData.Coverage = Convert.ToDecimal(dr["Count"].ToString());

                    cpData.Met = Convert.ToDecimal(dr["Met"].ToString());
                    cpData.Seen = Convert.ToDecimal(dr["Seen"].ToString());
