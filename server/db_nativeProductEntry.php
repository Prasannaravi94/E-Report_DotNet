...
function send_fcm_notify_ios( $reg_id, $message, $sf_name, $msg ) {
    ...
    define( "GOOGLE_API_KEY", "AAAAVR07Qn0:APA91bGb_80GnhEoxTanFoompVIDUKfgJKb_T4vdHF9sF5hzLGkET3UVTb46EGh_NbJwffAJ_Zx1MMhpDv-4Stp4VnmalWTnipph3mObNia3gj7h7jTALb1-5nDo15p7AE8JskEUO4Wr" );
    define( "GOOGLE_FCM_URL", "https://fcm.googleapis.com/fcm/send" );
    $fields = array(
        'registration_ids' => array( $reg_id ),
        'notification' => array( "body" => $msg, "title" => $sf_name, "msg" => $message, 'apns-push-type' => 'background', 'content-available' => '1' ), 'priority' => 'high' );
    /// 'notification' => array( 'sound' => 'default', 'body' => $message, 'title' => $sf_name, 'badge' => 12, 'content-available' => 'true', 'priority'=> 'high');
...
function actionLogin() {
    ...
    if ( $count == 1 ) {
        ...
        $respon[ 'ios_app' ] = $arr[ 0 ][ 'ios_app' ];
        $respon[ 'prod_remark' ] = 1;
        $respon[ 'ChmRxQty' ] = 1;
...
function svConversation_ios() {
    ...
    //	echo $query;
    send_push_message_ios( $sfcode, $msg, $sfcodefm, 0 );
    return ( $result );
}
...
function send_push_message( $sfcode, $msg, $sfcodefm, $type ) {
    if ( ( strpos( $sfcode, 'MR' ) !== false ) || ( strpos( $sfcode, 'MGR' ) !== false ) ) {
        ...
        $sql1 = "SELECT [Name] name, 'd5mPVwouQZGzpck4Zgijnv:APA91bGbadmQJUR3IxSSXeGKnnh9bz_MNZD72cYE2aTnEmHkdkkKzS8rsAQtDrMfttu9v2qMAInRibFRylabNoUjILpEhUoakRLtFbrVcoat2CFbeAAWOuKqZS9U13xF5jjGNhbpUJ99' reg_id FROM Mas_HO_ID_Creation WHERE HO_ID = '$sfcode'";
        $response_1 = performQuery( $sql1 );
        if ( ( strpos( $sfcodefm, 'MR' ) !== false ) || ( strpos( $sfcodefm, 'MGR' ) !== false ) ) {
            $sql2 = "SELECT Sf_Code code, Sf_Name name FROM Mas_Salesforce WHERE sf_code = '$sfcodefm'";
            $response_2 = performQuery( $sql2 );
...
function send_push_message_ios( $sfcode, $msg, $sfcodefm, $type ) {
    ...
    $messageWithoutSF = 'Hi, ' . $device_fm[ 0 ][ 'name' ] . ' has send you a Message' . '~' . $msg;
    $sf_name_fm = $device_fm[ 0 ][ 'name' ];
    send_fcm_notify_ios( $reg_id, $message, 'SAN CRM', $messageWithoutSF );
...
function addEntry() {
	...
	select '" . $tag_data[ 0 ][ 'SF_Code' ] . "','" . $tag_data[ 0 ][ 'Division_Code' ] . "','1','" . $tag_data[ 0 ][ 'OPD_Code' ] . "','Campaign','Apps','TR'";
        ...
        case "Doctor_Business_Entry":
            ...
            //$sql = "select isnull(max(Trans_sl_No),0)+1 sl_no from Trans_DCR_BusinessEntry_Head";
        ...
        case "Order_Product":
            ...
            break;
        case "Doctor_Business_Entry_old":
            $sfCode = $_GET[ 'sfCode' ];
            $sfName = $_GET[ 'sfName' ];
            $div = $_GET[ 'divisionCode' ];
            $divs = explode( ",", $div . "," );
            ...
            $tr = performQuery( $sql );
            $trans_slh = $tr[ 0 ][ sl_no ];
            $entryData = $pData[ 0 ][ "Doctor_Business_Entry" ];
            $entryDetail = $pData[ 1 ][ "Product_Details" ];
            $detail_No = $entryData[ 'Detail_No' ];
            //print_r( $entryData); die;
            if ( $detail_No != 0 ) {
                $sql1 = "delete from Trans_DCR_BusinessEntry_Head where Trans_sl_No = '$detail_No'";
                performQuery( $sql1 );
                //echo $sql1; die;
                $sql2 = "delete from Trans_DCR_BusinessEntry_Details where Trans_sl_No = '$detail_No'";
                performQuery( $sql2 );
            }

            $sql = "insert into Trans_DCR_BusinessEntry_Head (Sf_Code,Division_Code,Trans_Month,Trans_Year,Created_Date,Updated_Date,ListedDr_Name,ListedDrCode) select '$sfCode','$Owndiv','" . $entryData[ "Trans_Month" ] . "','" . $entryData[ "Trans_Year" ] . "','" . $entryData[ "createdDate" ] . "','" . $entryData[ "updatedDate" ] . "','" . $entryData[ "ListedDr_Name" ] . "','" . $entryData[ "ListedDrCode" ] . "'";
            performQuery( $sql );

            for ( $j = 0; $j < count( $entryDetail ); $j++ ) {
                $sql = "insert into Trans_DCR_BusinessEntry_Details (Trans_sl_No,Division_Code,ListedDrCode,Speciality_Code,Category_Code,Territory_Code,	Product_Code,Product_Quantity,MRP_Price,Retailor_Price,Distributor_Price,NSR_Price,Sample_Price,value,Product_Detail_Name,Product_Sale_Unit,Territory_Name,Target_Price) select '$trans_slh','$Owndiv','" . $entryDetail[ $j ][ "ListedDrCode" ] . "','" . $entryDetail[ $j ][ "Speciality_Code" ] . "','','" . $entryDetail[ $j ][ "Territory_Code" ] . "','" . $entryDetail[ $j ][ "Product_Code" ] . "','" . $entryDetail[ $j ][ "Product_Quantity" ] . "','" . $entryDetail[ $j ][ "MRP_Price" ] . "','" . $entryDetail[ $j ][ "Retailor_Price" ] . "','" . $entryDetail[ $j ][ "Distributor_Price" ] . "','" . $entryDetail[ $j ][ "NSR_Price" ] . "','" . $entryDetail[ $j ][ "Sample_Price" ] . "','" . $entryDetail[ $j ][ "value" ] . "','" . $entryDetail[ $j ][ "Product_Detail_Name" ] . "','" . $entryDetail[ $j ][ "Product_Sale_Unit" ] . "','" . $entryDetail[ $j ][ "Territory_Name" ] . "','" . $entryDetail[ $j ][ "Target_Price" ] . "'";
                echo $sql;
                performQuery( $sql );
            }
            break;
...
switch ( $value[ 0 ] ) {
    ...
    case "get_drbusiness_value":
        ...
        $divs = explode( ",", $div . "," );
        $Owndiv = ( string )$divs[ 0 ];
        $query = "select Trans_Sl_No,Sf_Code,Trans_Month,Trans_Year,Division_Code,CONVERT(varchar,Created_Date,21)Created_Date,ListedDr_Code,ListedDr_Name,Business_Value from Trans_Dr_Business_Valuewise_head  where Sf_Code='$sfCode' and Trans_Month='$Pmnth' and Trans_Year='$Pyear'";
        //echo $query;
        $results = performQuery( $query );
        outputJSON( $results );
        break;
