__w = window; lckMsg = ''; var count = 1; var clrcount = 1;
...
__Wndw = {
    ...
    A: { RwCtrl1: { half: { cap: "Half Day", ty: 'ta', id: "Half", h: 'fill', w: 'fill', mxl: 0 } }, prv_Cols: {} },
    P: {}
};
__Click = true;
__Alert = false;
...
(function ($) {
    DeviatealertBox = {
        BuildAlertdev: function () {
            //$("<div class='alrt-boxDev'><div class='alrt-contDev'><div class='alrt-HDDev'><b>Deviate Reason</b><a class='closBtDev' href='javascript:okClose();'>x</a></div><div class='MsgContDev'><input type=text class='inputDev' id=tb /></div><div class='MsgCont1'></div><div class='altr-ftrDev aBtnDev'><a id='aSCDev' href='javascript:okFunc();' class='button'>Save</a><a id='aDCDev' href='javascript:okFunDCls();' class='button'>OK</a></div><div class='altr-ftrDev cBtnDev'><a href='javascript:yFunc();' class='button button-green'>Yes</a><a href='javascript:nFunc();' class='button button-red'>No</a></div></div>").appendTo("body");
            $("<div class='alrt-boxDev'><div class='alrt-contDev'><div class='alrt-HDDev'><b>Deviate Reason</b></div><div class='MsgContDev'><input type=text class='inputDev' id=tb /></div><div class='MsgCont1'></div><div class='altr-ftrDev aBtnDev'><a id='aSCDev' href='javascript:okFunc();' class='button'>Save</a><a id='aDCDev' href='javascript:okFunDCls();' class='button'>OK</a></div><div class='altr-ftrDev cBtnDev'><a href='javascript:yFunc();' class='button button-green'>Yes</a><a href='javascript:nFunc();' class='button button-red'>No</a></div></div>").appendTo("body");
        },

        Show: function (msg, Fobj) {
            $('.alrt-boxDev').css('display', 'block');
            $('.aBtnDev').css('display', 'block');
            $('.aBtnDev').find('.button').focus();
            $('.cBtnDev').css('display', 'none');
            $('.inputDev').css('display', 'block');
            $('.MsgCont1').html("");
            $('#aDCDev').hide();
            $('#aSCDev').show();
            __Alert = true;
            ...
            okFunc = function () {
                ...
                var txtDe = $("#tb").val();
                if ($.trim(txtDe) == '') {
                    $('.MsgCont1').html("Entry The Deviate Reason");
                    return false;
                }

                $('.inputDev').css('display', 'none');
                DeviateValidate();
                $('#aDCDev').show();
                $('#aSCDev').hide();
                $('.MsgCont1').html("Reason Save Successfully");
                $("#chkDeviate").attr("checked", false);

                if (Setup.TPDCR_MGRAppr == 1) {
                    //window.location.href = "DCR_Entry.aspx";
                    //location.reload();
                    document['location']['href'] = 'DCR_Entry.aspx';
                }
            }
            okClose = function () {
                $('.MsgCont1').html("");
                DeviatealertBox.hide();
            }
        ...
        hide: function () {
            $('.inputDev').val("");
            $('.alrt-boxDev').css('display', 'none');
            __Alert = false;
        },
        confirm: function (msg, tFunc, fFunc) {
            alertBox.Show(msg);
            $('.cBtnDev').css('display', 'block');
            $('.cBtnDev').find('.button-green').focus();
            $('.aBtnDev').css('display', 'none');
            yFunc = function () {
                alertBox.hide();
                $(tFunc);
            }
...
$(document).ready(function () {
    ...
    $.maxlengthNumber();
    alertBox.BuildAlert();
    DeviatealertBox.BuildAlertdev();
    console.log(check);
    for (var value of check) {

        $('#checkbx').append("<input type='checkbox' name='" + value.Name + "' id='" + value.Code + "' value='" + value.Code + "'>")
          .append("<label for='" + value.Code + "'>" + value.Name + "</label>")
          .append("<br>");
    }
    ...
    $(window).load(function () {
        ...
        if (DCRDatas.SFCode != undefined && SFDet.SFCode == DCRDatas.SFCode && $('#hDCRDt').val() == DCRDatas.EDate) {
            ...
            if (DCRDatas.Half != undefined) $("#txa_AHalf").val(DCRDatas.Half);
            ChangeWT();
            ChangeHQ();
        }
        else {
        ...
        if (Setup.TPBaseDCRDr != 1) setData('DCRMain', DCRDatas);
        DCR.SFCode = DCRDatas.SFCode;
        DCR.EDate = DCRDatas.EDate;
        ...
        var ddl = $("[id*=ddl_WorkType]");
        var selectedText = ddl.find("option:selected").text();
        var Hty = $('#hDtTyp').val();

        if (Setup.TPBaseDCRDr == 1 && selectedText != '-- Select The Work Type --' && (Hty == 0 || Hty == 3 || Hty == 4)) {
            $("#ddl_WorkType").attr("disabled", true);
            if (Setup.No_of_TP_View == 1)
                $("#ddl_SDP").attr("disabled", true);

            $("#ddl_HQ").attr("disabled", true);
            if (Setup.TPDCRDeviation == 1) {
                $("#chkDeviate").show();
                $("#SpnDeviate").show();
                $(".plnDeivate").css('display', 'inline-block');
            }
        ...
        terrtype = $("#hTerrType").val();
        SFty = $("#hSFTyp").val();

        if (Setup.TPBaseDCRDr == 1 && terrtype == 1 && SFty == 1) {
            $("#ddl_SDP").attr("disabled", false);
        }
        else if (Setup.TPBaseDCRDr == 1 && terrtype == 1 && SFty == 2) {
            $("#ddl_HQ").attr("disabled", false);
        }
        ...
        if ($("#hSRem").val().indexOf("Rejection") >= 0 || $("#hSRem").val().indexOf("Edit") >= 0) {
            if (Setup.ProdSampleQtyValid == 1 && Setup.InputQtyValid == 1)
                alert("For Edit/Re-entry,Kindly Again Enter the Sample/Input Qty for Selected Product/Input");
            if (Setup.ProdSampleQtyValid == 1 && Setup.InputQtyValid == 0)
                alert("For Edit/Re-entry,Kindly Again Enter the Sample Qty for Selected Product");
            if (Setup.ProdSampleQtyValid == 0 && Setup.InputQtyValid == 1)
                alert("For Edit/Re-entry,Kindly Again Enter the Input Qty for Selected Input");
    ...
    $("#txa_AHalf").on("paste", function (e) {
        if (!e) e = event;
        var pastedData = e.originalEvent.clipboardData.getData('text');
        window.document.execCommand('insertText', false, pastedData.replace(/\'/g, "").replace(/\"/g, ""));
        return false;
    });
    $("#txa_AHalf").keydown(function (event) {
        keys = [222]; if ($.inArray(event.which, keys) > -1) return false;
    });
    $("#txa_AHalf").on('keyup',
    function (e) {
        DCRDatas.Half = $(this).val();
        setData('DCRMain', DCRDatas);
    });
    $("#tCusHalf").on("paste", function (e) {
        if (!e) e = event;
        var pastedData = e.originalEvent.clipboardData.getData('text');
        window.document.execCommand('insertText', false, pastedData.replace(/\'/g, "").replace(/\"/g, ""));
        return false;
    });
    $("#tCusHalf").keydown(function (event) {
        keys = [222]; if ($.inArray(event.which, keys) > -1) return false;
    });
...
var DCR = {
    ...
    WrkWin: {
        create: function (a) {
            for (ix in a) {
                ...
                if (a[ix].RwCtrl1 != undefined) {
                    eTab = ''; eDtD = '';

                    for (_ic in a[ix].RwCtrl1) {
                        //console.log(check);
                        fa = a[ix].RwCtrl1[_ic];
                        if (fa.m == undefined) fa.m = 0;
                        ...
                        if (fa.m == 0) {
                            eDtD += '<div style="padding-bottom:4px;">';
                            if (fa.cap != undefined) eTab += "<span style='display:block;width:" + ((fa.w == "fill") ? "98.5%" : (fa.w + "px")) + " !important;' class='GrdH'>" + fa.cap + "<span class='ntpad'></span></span>";
                            if (fa.ty == 'ta') eDtD += ' <div id="checkbx" style="width:20%"> </div>';
                            eDtD += '</div>';
                            //var assetList = $('#checkboxlist')
                            ...
                            //    $("#chkhaf_work").append($("<option/>").val(this.Code).text(this.Name));
                            //});
                            //for (var value of check) {

                            //    $('#checkbx').append("<input type='checkbox' id='" + value.Code + "' value='" + value.Code + "'>")
                            //      .append("<label for='"+value.Code+"'>2</label>")
                            //      .append("<br>");
                            //}
                        }
                        //reloadDatas('A');
                        //srtCusTwbs($("#ddl_check"));
    ...
    validEntry: function (typ) {
        ...
        shalf = $('#ddl_' + typ + 'half').attr('data-value') || "";
        ...
        //if (typ == "D" && _st.half.D > 0 && Setup.DrHalf > 0 && (shalf == '' && sfedbk == '')) {
        //    ddlWinOpen($("#ddl_Dhalf"), $("#ddl_Dhalf").find('.disp-selopt'), $("#wHalf"));
        //    return false;
        //}

        var inpu_pv = $("#ddl_Dinp").attr("data-value");
        if (typ == "D" && inpu_pv == '' && Setup.InputMand == 1) {
            ddlWinOpen($("#ddl_Dinp"), $("#ddl_Dinp").find('.disp-selopt'), $("#wInput"));//Input selection popup window when click go button
            alertBox.Show('Enter the Input');
            return false;
        }
    },
    insEntry: function (typ) {
        ...
        for (wky in _C) {
            if (wky == "rem") {
                ...
                cust.half = $('#ddl_' + typ + 'half').attr('data-value') || "";
                cust.fedbk = {};
                cust.fedbk.val = $('#ddl_fedBk').attr('data-value') || "";
                cust.fedbk.txt = $('#ddl_fedBk').attr('data-text') || "";
            }
            ...
            if (mx != '') {
                ...
                for (il = 0; il < mxvs.length - 1; il++) {
                    if (mxvs[il] != '') {
                        ...
                        $('#tProd li').each(function () {
                            var dataId = $(this).attr("data-val");
                            var datatxt = $(this).text();
                            if (dataId == val) {


                                sampletxt = datatxt.split('(')
                                sampletxts = sampletxt[1].split(')')

                                if (sampletxts[0] != undefined) {

                                    a = parseInt(sampletxts[0], 10) - parseInt(sampleQty, 10);

                                    $(this).html("<label>" + sampletxt[0].replace(/-/g, '') + " (" + a + ")" + "</label>");
            ...
            if (Imx != '') {
                ...
                for (il = 0; il < Imxvs.length - 1; il++) {
                    if (Imxvs[il] != '') {
                        ...
                        $('#tInput li').each(function () {
                            var dataId = $(this).attr("data-val");
                            var datatxt = $(this).text();
                            if (dataId == IVal) {
                                Inputtxt = datatxt.split('(')
                                Inputtxts = Inputtxt[1].split(')')
                                if (Inputtxts[0] != undefined) {
                                    a = parseInt(Inputtxts[0], 10) - parseInt(InputQty, 10);
                                    $(this).html("<label>" + Inputtxt[0].replace(/-/g, '') + " (" + a + ")" + "</label>");
                                }
                            }
                        });
                    }
        ...
        if (data.length > 0) {
            ...
            for (wky in __Wndw[typ].fields) {
                if (wky == "rem") {
                    ...
                    tx = cust.fedbk.txt;
                    $('#ddl_' + typ + 'half').setItem(cust.half, cust.half);
                    $('#ddl_' + typ + 'half').find('span').text(tx + ', ' + cust.half);
                    $('#tCusHalf').val(cust.half);
                    $('#ddl_fedBk').setItem(cust.fedbk.val, tx);
                }
                else if (wky != "go") {
                        ...
                        if (Imxvs[il] != '') {
                            ...
                            $('#tInput li').each(function () {
                                ...
                                if (dataId == Ival) {
                                    ...
                                    Inputtxts = Inputtxt[1].split(')')
                                    if (Inputtxts[0] != undefined) {
                                        b = parseInt(Inputtxts[0], 10) + parseInt(InputQty, 10);
                                        $(this).html("<label>" + Inputtxt[0].replace(/-/g, '') + " (" + b + ")" + "</label>");
                                    }
                                }
                            });
                        }
        ...
        for (ij = 0; ij < data.length; ij++) {
            ...
            for (wky in __Wndw[typ].grd_Cols) {
                ...
                if (wky == "go") {
                    ...
                    //sgrd += "padding-left: 10px;'>" + ((wky == "half") ? gRW.fedbk.txt + ', ' + gRW[wky] : ((gRW[wky] == undefined) ? "" : (gRW[wky].txt == undefined) ? gRW[wky] : gRW[wky].txt))
                }
                sgrd += "</td>";
            }
        ...
        alertBox.confirm("Do you want to <b style='color:red'>Clear Data(s)</b> for this <b>" + $("#DtInf").html() + "</b> Date ?", function () {
            alertBox.confirm("Are you sure to clear data(s)?", function () {
                for (i = localStorage.length - 1; i > -1; i--) {
                    ...
                    if (key.indexOf('dta_' + DCR.SFCode + '_' + DCR.EDate.replace(/\//g, '_') + '_') > -1) {


                        if (clrcount == 1 && (Setup.InputQtyValid == 1 || Setup.ProdSampleQtyValid == 1)) {
                            SampleInputCalc(0);
                            clrcount = 0;
                        }
                        delData(key);
                ...
                for (ij = 0; ij < data.length; ij++) {
                    ...
                    for (wky in __Wndw[typ].prv_Cols) {
                        ...
                        if (wky != "go") {
                            ...
                            //sgrd += "<td style='border:1px solid #D2D2D2;min-width:" + w + "px !important;max-width:" + w + "px !important;width:" + w + "px !important;padding-left: 10px;'>" + ((wky == "half") ? gRW.fedbk.txt + ', ' + gRW[wky] : ((gRW[wky] == undefined) ? "" : (gRW[wky].txt == undefined) ? gRW[wky] : gRW[wky].txt)) + "</td>";
                        }
    ...
    validDCR: function () {
        ...
        if ($('#chkDeviate').is(':checked')) { //Newly added 
            showAlert("Enter Deviate Reason....");
            return false;
        }
    ...
    svSaveDCR: function () {
        ...
        debugger
        selValues = "";
        var ChkboxList1Ctl = document.getElementById("checkbx");
        var itemarr = document.getElementById("checkbx").getElementsByTagName("span");
        var ChkboxList1Arr = document.getElementById("checkbx").getElementsByTagName("input");

        if (ChkboxList1Arr != null) {
            for (var i = 0; i < ChkboxList1Arr.length; i++) {
                if (ChkboxList1Arr[i].checked)
                    selValues += ChkboxList1Arr[i].name + "/";
            }
            if (selValues.length > 0)
                selValues = selValues.substr(0, selValues.length - 1);
        }
        //DCRDta.halfday = $("#hSFTyp").val();
        ...
        PageMethods.SaveDCR(SFData, DCRData, sSetup, selValues, nlst, function (data) {
...
setSS = function () { $("#alSS").attr("href", $("#aSS").attr("href")) }
showHideBlockMsg = function () {
    $('#lckMsg').css('display', ((lckMsg != '') ? 'block' : 'none'));
    $('#frm').css('display', ((lckMsg != '') ? 'none' : 'block'));
    $('#lckMsg').html("<div class='alert alert-danger'><strong>" + lckMsg + "</strong></div><a id='alSS'  onclick='setSS()' class='button home-bt'>SS</a>")
}


setIS = function () { $("#alIS").attr("href", $("#aIS").attr("href")) }
showHideBlockMsg = function () {
    $('#lckMsg').css('display', ((lckMsg != '') ? 'block' : 'none'));
    $('#frm').css('display', ((lckMsg != '') ? 'none' : 'block'));
    $('#lckMsg').html("<div class='alert alert-danger'><strong>" + lckMsg + "</strong></div><a id='alSS'  onclick='setIS()' class='button home-bt'>SS</a>")
}
...
ddlWinOpen = function (ddl, $x, ddlw) {
    ...
    if (Setup.PrdSampleQty == 0 && ($('.wTab.active').attr('id') == 'idWD' || $('.wTab.active').attr('id') == 'idWU')) {
        $("#idTPQH").hide();
        $("#idtPQ").hide();
    }
    else if (Setup.PrdSampleQty == 1 && ($('.wTab.active').attr('id') == 'idWD' || $('.wTab.active').attr('id') == 'idWU')) {
        $("#idTPQH").show();
        $("#idtPQ").show();
        document.getElementById('lblSampleQty').innerHTML = Setup.DrSampleQtyCaption;
    }
    if (Setup.ChemSampleQty == 0 && ($('.wTab.active').attr('id') == 'idWC' || $('.wTab.active').attr('id') == 'idWS')) {
        $("#idTPQH").hide();
        $("#idtPQ").hide();
    }
    else if (Setup.ChemSampleQty == 1 && ($('.wTab.active').attr('id') == 'idWC' || $('.wTab.active').attr('id') == 'idWS')) {
        $("#idTPQH").show();
        $("#idtPQ").show();
        document.getElementById('lblSampleQty').innerHTML = Setup.ChemSampleQtyCaption;
    }
    // if (Setup.DRxQty == 0 || $('.wTab.active').attr('id') != 'idWD') {
    if (Setup.DRxQty == 0 && ($('.wTab.active').attr('id') == 'idWD' || $('.wTab.active').attr('id') == 'idWU')) {
        //$(ddlw).find('.TPVH').css('display', 'none');
        //$(ddlw).find('.tPV').parent().css('display', 'none');
        $("#idTPVH").hide();
        $("#idtPV").hide();
    }
    if (Setup.ChemPOBQty == 0 && ($('.wTab.active').attr('id') == 'idWC' || $('.wTab.active').attr('id') == 'idWS')) {
        $("#idTPVH").hide();
        $("#idtPV").hide();
    }
    else if (Setup.ChemPOBQty == 1 && ($('.wTab.active').attr('id') == 'idWC' || $('.wTab.active').attr('id') == 'idWS')) {
        $("#idTPVH").show();
        $("#idtPV").show();
        document.getElementById('lblQty').innerHTML = Setup.ChemQtyCaption;
    }
    if (Setup.DRxQty == 1 && ($('.wTab.active').attr('id') == 'idWD' || $('.wTab.active').attr('id') == 'idWU')) {
        $("#idTPVH").show();
        $("#idtPV").show();
        document.getElementById('lblQty').innerHTML = Setup.ProdQtyCaption;
    }
...
setData = function (ky, Data) {
    _lcStore.setItem(ky, JSON.stringify(Data));
}
    ...
    if (vals != '') {
        ...
        for (il = 0; il < spvs.length - 1; il++) {
            ...
            if (tid == "tProd") {
                ...
                //Newly Added for sample valid
                spt[0] = $(ddl).find("li[data-val='" + spv[0] + "']").text();
...
svWinVal = function (e) {
    ...
    if (tb.attr("id") == "tProd") {
        $("#tProd").find(".Prod").each(function () {
            ...
            if (Setup.DRxFed == 1) {
                if ($r.find('.PRx').attr('data-text') == "- Select the Rx Feedback -") {
                    alertBox.Show('Select the FeedBack', $r.find(".disp-selopt"));
                    vld = false;
                    return false;
                }
            }
            ...
            //    return false;
            //}
            if (Setup.PQtyZro == 1 && $r.find('.tPQ').val() == "" && ($('.wTab.active').attr('id') == 'idWD' || $('.wTab.active').attr('id') == 'idWU'))
                $r.find('.tPQ').val('0');
            else if (Setup.PQtyZro == 0 && Setup.PrdSampleQty == 1 && $r.find('.tPQ').val() == "" && ($('.wTab.active').attr('id') == 'idWD' || $('.wTab.active').attr('id') == 'idWU')) {
                alertBox.Show('Enter the ' + Setup.DrSampleQtyCaption, $r.find(".tPQ")); vld = false;
                return false;
            }

            if (Setup.DrPOBQtyZero == 1 && $r.find('.tPV').val() == "" && ($('.wTab.active').attr('id') == 'idWD' || $('.wTab.active').attr('id') == 'idWU'))
                $r.find('.tPV').val('0');
            else if (Setup.DrPOBQtyZero == 0 && Setup.DRxQty == 1 && $r.find('.tPV').val() == "" && ($('.wTab.active').attr('id') == 'idWD' || $('.wTab.active').attr('id') == 'idWU')) {
                alertBox.Show('Enter the ' + Setup.ProdQtyCaption, $r.find(".tPV")); vld = false;
                return false;
            }
            if (Setup.ChemSampleQtyZero == 1 && Setup.ChemSampleQty == 1 && $r.find('.tPQ').val() == "" && ($('.wTab.active').attr('id') == 'idWC' || $('.wTab.active').attr('id') == 'idWS' || $('.wTab.active').attr('id') == 'idWH'))
                $r.find('.tPQ').val('0');
            else if (Setup.ChemSampleQtyZero == 0 && $r.find('.tPQ').val() == "" && ($('.wTab.active').attr('id') == 'idWC' || $('.wTab.active').attr('id') == 'idWS' || $('.wTab.active').attr('id') == 'idWH')) {
                alertBox.Show('Enter the ' + Setup.ChemSampleQtyCaption, $r.find(".tPQ")); vld = false;
                return false;
            }
            if (Setup.ChemPOBQtyZero == 1 && $r.find('.tPV').val() == "" && ($('.wTab.active').attr('id') == 'idWC' || $('.wTab.active').attr('id') == 'idWS' || $('.wTab.active').attr('id') == 'idWH'))
                $r.find('.tPV').val('0');
            else if (Setup.ChemPOBQtyZero == 0 && Setup.ChemPOBQty == 1 && $r.find('.tPV').val() == "" && ($('.wTab.active').attr('id') == 'idWC' || $('.wTab.active').attr('id') == 'idWS' || $('.wTab.active').attr('id') == 'idWH')) {
                alertBox.Show('Enter the ' + Setup.ChemQtyCaption, $r.find(".tPV")); vld = false;
                return false;
            }
            ...
            //New
            if (Setup.ProdSampleQtyValid == 1) {
                Sample = $(this).attr('data-text').split('(')
                if (Sample[1] != undefined) {
                    SampleQty = Sample[1].split(")");


                    if (SampleQty[0] != undefined) {
                        if (parseInt($r.find('.tPQ').val(), 10) > parseInt(SampleQty[0], 10))   // if (a > 5)                     
                        {
                            alertBox.Show($(this).attr('data-text') + ' Sample Qty Exceeded'); vld = false;
                            return false;
                        }
                    }
                }
            ...
            lv += $(this).attr('data-value') + '~' + $r.find('.tPQ').val() + '$' + $r.find('.tPV').val() + '$' + $r.find('.PRx').attr('data-value') + "#";
        	//tx += $(this).attr('data-text') + ' ( ' + $r.find('.tPQ').val() + ' )';
            tx += $(this).attr('data-text').replace(/ *\([^)]*\) */g, "").trim() + ' ( ' + $r.find('.tPQ').val() + ' )';
            if (Setup.DRxQty == 1 && $('.wTab.active').attr('id') == 'idWD') tx += ' ( ' + $r.find('.tPV').val() + " )";
            if (Setup.DRxFed == 1 && $('.wTab.active').attr('id') == 'idWD') tx += ' ( ' + $r.find('.PRx').attr('data-text') + " )";
            tx += ', ';
        })
    }
    ...
    if (tb.attr("id") == "tInput") {
        $("#tInput").find(".ddl-Box").each(function () {
            ...
            if ($r.find('.tGQ').val() == "" || $r.find('.tGQ').val() == "0" && $(this).attr('data-value') != "-1") {
                alertBox.Show('Enter the Qty greater than 0', $r.find(".tGQ")); vld = false;
                return false;
            }
            if (('#' + lv).indexOf('#' + $(this).attr('data-value') + '~') > -1) {
            ...
            if (Setup.InputQtyValid == 1) {
                ...
                Input = $(this).attr('data-text').split('(')
                if (Input[1] != undefined) {
                    InputQty = Input[1].split(")");


                    if (InputQty[0] != undefined) {
                        if (parseInt($r.find('.tGQ').val(), 10) > parseInt(InputQty[0], 10)) {
                            alertBox.Show($(this).attr('data-text') + ' Input Qty Exceeded'); vld = false;
                            return false;
                        }
                    }
                }
            ...
            tx += $(this).attr('data-text').replace(/ *\([^)]*\) */g, "").trim() + ' ( ' + $r.find('.tGQ').val() + " ), ";
        })
    }
    if ($(win).attr("id") == "wRem") {
        lv = $('#tCusRem').val();
        tx = $('#ddl_fedBk').attr('data-text') + ', ' + $('#tCusRem').val();
    }
    if ($(win).attr("id") == "wHalf") {
        lv = $('#tCusHalf').val();
        tx = $('#ddl_fedBk').attr('data-checked') + ', ' + $('#tCusHalf').val();
    }
    if (vld == true) {
...
initialize = function () {
    ...
    //__Wndw.R.RwCtrl1.half.mxl = Setup.HalfLen;
...
BuildMenu = function (flg) {
    ...
    if (sFWFl == 'F' && ((sfty == 1 && flg == 0) || sfty != 1 && flg == 1)) getMasterList();
    //}    
}
...
reloadTwns = function (sf) {
    ...
    //if (Setup.TPBaseDCRDr == 1)
    //    $('#ddl_SDP').val(sTPDCR.Territory_Code);

    //if (Setup.TPBaseDCRDr == 1 && $("#hSRem").val().indexOf("TP") == -1) {   //TP based on Territory area
    //    $('#ddl_SDP option').each(function () {
    //        var datavalue = $(this).attr("value");
    //        if (sTPDCR.Territory_Code.indexOf(datavalue) == -1) {
    //            $(this).remove();
    //        }
...
/* Deviate Changes */
chkActivate = function (sf) {
    if (Setup.TPDCRDeviation == 1) {

        if ($('#chkDeviate').is(':checked')) {

            $("#ddl_WorkType").attr("disabled", false);
            $("#ddl_HQ").attr("disabled", false);
            $("#ddl_SDP").attr("disabled", false);
            DeviatealertBox.Show("select");
        }
        else {
            $("#ddl_WorkType").attr("disabled", true);
            $("#ddl_HQ").attr("disabled", true);
...
DeviateValidate = function () {
    var SFCode = SFDet.SFCode;
    var EDate = $('#hDCRDt').val();
    var DivCode = SFDet.Div;
    var SFName = SFDet.SFName;

    var Reason = $("#tb").val();
    sFWFl = $('#ddl_WorkType').find('option:selected').text();
    sTerritory = $('#ddl_SDP').find('option:selected').text();
    sTerritory_Code = $('#ddl_SDP').val();

    //PageMethods.SaveDeviate(SFCode, EDate, DivCode, Reason, sTerritory, sFWFl, SFName, Setup.TPDCR_MGRAppr, sTerritory_Code);

    var Data = SFCode + "^" + EDate + "^" + DivCode + "^" + Reason + "^" + sTerritory + "^" + sFWFl + "^" + SFName + "^" + Setup.TPDCR_MGRAppr + "^" + sTerritory_Code;
    $['ajax']({
        type: "POST",
        url: "DCR_Entry.aspx/SaveDeviate",
        async: false,
        data: '{objData:' + JSON.stringify(Data) + '}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            //createCustomAlert('Record Added Successfully');
...
srtCusTwbs = function (x) {
    ...
    var $val = $("#ddl_SDP option:selected").text();
    var result = $val.indexOf("Visit Exceeded");
    if (result >= 0) {
        alertBox.Show("This Territory already Visited as per visit Frequency");
        $("#ddl_SDP").val(" ");
        return false;
    }
...
$(document).ready(function () {
    ...
    $("#ddl_Dcus").on("click", ".active-result", function (event) {

        var prd_clrcode = $(this).attr("data-prd_clrcode");

        var numbers = prd_clrcode.split(',');

        $("#tProd li").removeClass("prdclr");

        $('#tProd ul').addClass('sortcolor');
        ...
        for (var i = 0; i < numbers.length; i++) {
            ...
            $('#tProd li').each(function () {
                ...
                if (dataId == numbers[i]) {
                    ...
                    //$(this).css("backgroundColor", "#c9dc59");
                    $(this).addClass('prdclr');
                }

            });
        }

        $('.prdclr').prependTo('.sortcolor');
...
$(document).ready(function () {
    $("#ddl_Dprd").on("click", function (event) {




        $('#tProd tr').each(function () {
            var unique = {},
             dataItemId = -1;

            $(this).find('ul li').each(function () {

                dataItemId = $(this).attr('data-val');
        ...
        //var div_code = $('#hDiv').val();

        //var SFty = $("#hSFTyp").val();


        //if (div_code == '65' && SFty == '1') {

        //    $('#tProd ul li').hide();
        //    $('#tProd ul li.prdclrRx').show();
        //    $('#tProd ul li.prdclr').show();
        //}

    });
    var div_code = $('#hDiv').val();
    var sf_type = $('#hSFTyp').val();
    if (sf_type == '1' && (div_code == '2' || div_code == '4' || div_code == '1' || div_code == '6')) {

        $("#ddl_SDP").on("click", function (event) {

            var dateTime = $('#hDCRDt').val();
            // var dateTime = '8/10/2019'
            ...
            var prev_month = previous.getMonth();
            var prev_year = previous.getFullYear();

            if (cur_month == month && year == cur_year) {

                $('#ddl_SDP option').each(function () {
                    var CM_Visit = $(this).attr("data-cmvisit");
                    var datavalue = $(this).attr("data-value");

                    if (CM_Visit == '0') {

                        //$(this).text($(this).text() + " (Visit Exceeded )");
                        //$('li').text('Visit Exceeded');
                        $('#ddl_SDP option[name="' + $(this).text() + '"]').text($(this).text() + " ( Visit Exceeded )");
                        //$(this).find('select option').eq(0).text($(this).text() + " ( Visit Exceeded )");
                        $(this).css("backgroundColor", "#f0ad4e");
                        $(this).attr("data-val", "");


                        $(this).click(function (e) {
                            // alert("Visit Exeeded - " + $(this).text());
                            $("#ddl_SDP span").text("Visit Exeeded - " + $(this).text());

                        });
                    }
                });
            }
            else if (prev_month == month && year == prev_year) {

                $('#ddl_SDP option').each(function () {
                    var PM_Visit = $(this).attr("data-pmvisit");
                    var datavalue = $(this).attr("data-value");

                    if (PM_Visit == '0') {

                        // $(this).text($(this).text() + " (Visit Exceeded )");
                        //var a = $(this).find('label').eq(0).text($(this).text() + " ( Visit Exceeded )");
                        //$(this).find('label').eq(0).text($(this).text() + " ( Visit Exceeded )");
                        $('#ddl_SDP option[name="' + $(this).text() + '"]').text($(this).text() + " ( Visit Exceeded )");
                        $(this).css("backgroundColor", "#f0ad4e");
...
SampleInputCalc = function (SIData) {
    mx = '';
    Imx = '';
    sopt = $('#ddl_WorkType').find('option:selected');
    otabs = sopt.data('etabs');
