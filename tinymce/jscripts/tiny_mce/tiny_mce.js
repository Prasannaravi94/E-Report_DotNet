...
(function () {
    ...
    tinymce.create('tinymce.util.URI', {
        ...
        toRelPath: function (base, path) {
            ...
            if (bp == 1) return path;
            for (i = 0; i < base.length - (bp - 1); i++) out += "../";
...
tinymce.create('static tinymce.util.JSON', {
    serialize: function (o) {
        var i, v, s = tinymce.util.JSON.serialize,
            t;
    ...
    tinymce.create('tinymce.dom.DOMUtils', {
        ...
        add: function (p, n, a, h, c) {
            ...
            return this.run(p, function (p) {
                ...
                if (a) {
                    for (k in a) {
                        if (a.hasOwnProperty(k) && !is(a[k], 'object')) t.setAttrib(e, k, '' + a[k]);
                    }
                ...
                switch (n) {
                    case "style":
                        if (s.keep_values && /^(| )(top|left|bottom|right|width|height)/i.test(v)) {
                            if (v) e.setAttribute('mce_style', v, 2);
            ...
            if (s.keep_values) {
                ...
                h = h.replace(/<([\w:]+) [^>]*(src|href|style|coords)[^>]*>/gi, function (a, n) {
                    function handle(m, b, c) {
                        ...
                        if (b == 'style') {
                            if (/^(| )(top|left|bottom|right|width|height)/i.test(c)) return m;
                            if (s.hex_colors) {
...
(function () {
    ...
    tinymce.create('tinymce.dom.Selection', {
        ...
        getContent: function (s) {
            ...
            } else if (is(r.item) || is(r.htmlText)) e.innerHTML = r.item ? r.item(0).outerHTML : r.htmlText;
        ...
        setContent: function (h, s) {
            ...
            if (r.insertNode) {
                ...
                try {
                    if (d.queryCommandEnabled('InsertHTML')) return d.execCommand('InsertHTML', false, h);
                } catch (ex) {
                    r.deleteContents();
                    r.insertNode(t.getRng().createContextualFragment(h));
                }
            } else {
                if (r.item) r.item(0).outerHTML = h;
                else r.pasteHTML(h);
            }
        },
        getStart: function () {
        ...
        getBookmark: function (si) {
            var t = this,
                r = t.getRng(),
                tr, sx, sy, vp = t.dom.getViewPort(t.win),
                e, sp, bp, le, c = -0xFFFFFF,
                s, ro = t.dom.getRoot(),
                wb = 0,
                wa = 0,
                nv;
            ...
            function getPos(r, sn, en) {
                var w = t.dom.doc.createTreeWalker(r, NodeFilter.SHOW_TEXT, null, false),
                    n, p = 0,
                    d = {};
            ...
            if (s.anchorNode == s.focusNode && s.anchorOffset == s.focusOffset) {
                ...
                (s.anchorNode.nodeValue || '').replace(/^\s+/, function (a) {
                    wb = a.length;
                });
                return {
                ...
                (r.startContainer.nodeValue || '').replace(/^\s+/, function (a) {
                    wb = a.length;
                });
                (r.endContainer.nodeValue || '').replace(/^\s+/, function (a) {
                    wa = a.length;
                });
                if (!e) return {
                    scrollX: sx,
    ...
    tinymce.create('tinymce.dom.Serializer', {
        ...
        _serializeNode: function (n, inn) {
            var t = this,
                s = t.settings,
                w = t.writer,
                hc, el, cn, i, l, a, at, no, v, nn, ru, ar, iv;
        ...
        _setup: function () {
            ...
            if (s.invalid_elements) t.invalidElementsRE = new RegExp('^(' + wildcardToRE(s.invalid_elements.replace(',', '|').toLowerCase()) + ')$');
...
(function () {
    ...
    tinymce.create('tinymce.ui.Button:tinymce.ui.Control', {
        ...
        renderHTML: function () {
            ...
            if (s.image) h += '<img class="mceIcon" src="' + s.image + '" />' + l + '</a>';
            else h += '<span class="mceIcon ' + s['class'] + '"></span>' + (l ? '<span class="' + cp + 'Label">' + l + '</span>' : '') + '</a>';
            return h;
        },
        postRender: function () {
            var t = this,
        ...
        select: function (v) {
            ...
            if (v != t.selectedValue) {
                ...
                if (!fv) {
                    ...
                    e = 0;
                    return;
                } else DOM.removeClass(e, 'mceTitle');
            }
            e = 0;
        },
        add: function (n, v, o) {
...
tinymce.create('tinymce.ui.Toolbar:tinymce.ui.Container', {
    renderHTML: function () {
        ...
        for (i = 0; i < cl.length; i++) {
            ...
            if (dom.stdMode) h += '<td style="position: relative">' + co.renderHTML() + '</td>';
    ...
    tinymce.create('tinymce.AddOnManager', {
        ...
        requireLangPack: function (n) {
            ...
            if (tinymce.EditorManager.settings) {
                ...
                if (s) {
                    if (!tinymce.dom.Event.domLoaded && !s.strict_mode) tinymce.ScriptLoader.load(u);
...
(function () {
    ...
    tinymce.create('static tinymce.EditorManager', {
        ...
        preInit: function () {
            ...
            if (tinymce.EditorManager.baseURI.host != lo.hostname && lo.hostname) document.domain = tinymce.relaxedDomain = lo.hostname.replace(/.*\.(.+\..+)$/, '$1');
                    ...
                    case "exact":
                        ...
                        if (l.length > 0) {
                            each(explode(l), function (v) {
                                if (DOM.get(v)) new tinymce.Editor(v, s).render(1);
                                else {
                                    c = 0;
...
(function () {
    ...
    tinymce.create('tinymce.Editor', {
        ...
        render: function (nst) {
            ...
            if (s.encoding == 'xml') {
                t.onGetContent.add(function (ed, o) {
                    if (o.get) o.content = DOM.encode(o.content);
                });
            }
                    ...
                    if (s.ask) {
                        function ask() {
                            t.windowManager.confirm(t.getLang('edit_confirm'), function (s) {
                                if (s) t.init();
                                else Event.remove(t.id, 'focus', ask);
                            });
                        };
        ...
        init: function () {
            ...
            if (s.popup_css) s.popup_css = t.documentBaseURI.toAbsolute(s.popup_css);
            else s.popup_css = t.baseURI.toAbsolute("themes/" + s.theme + "/skins/" + s.skin + "/dialog.css");
            ...
            if (tinymce.relaxedDomain) {
                if (isIE) u = 'javascript:(function(){document.open();document.domain="' + document.domain + '";var ed = window.parent.tinyMCE.get("' + t.id + '");document.write(ed.iframeHTML);document.close();ed.setupIframe();})()';
        ...
        _addEvents: function () {
            ...
            each(lo, function (v, k) {
                switch (k) {
                    case 'contextmenu':
                        ...
                        } else Event.add(t.getDoc(), k, eventHandler);
                        break;
                    case 'paste':
                ...
                function setOpts() {
                    ...
                    if (isGecko) {
                        ...
                        try {
                            ...
                            if (!t._isHidden()) d.execCommand("useCSS", 0, true);
                        }
            ...
            t.onKeyUp.add(function (ed, e) {
                if ((e.keyCode >= 33 && e.keyCode <= 36) || (e.keyCode >= 37 && e.keyCode <= 40) || e.keyCode == 13 || e.keyCode == 45 || e.keyCode == 46 || e.keyCode == 8 || e.ctrlKey) t.nodeChanged();
            });
        ...
        _convertFonts: function () {
            ...
            function convertToFonts(no) {
                var n, f, nl, x, i, v, st;
                if (tinymce.isWebKit || !s.inline_styles) return;
                ...
                for (x = nl.length - 1; x >= 0; x--) {
                    ...
                    if (sl) {
                        ...
                        if (i == -1 && v.indexOf('pt') > 0) i = inArray(fz, parseInt(v));
                        if (i == -1) i = inArray(fzn, v);
                        if (i != -1) {
                            dom.setAttrib(f, 'size', '' + (i + 1 || 1));
                            f.style.fontSize = '';
                        }
    ...
    tinymce.create('tinymce.EditorCommands', {
        ...
        mceJustify: function (c, v) {
            ...
            if (nn == 'IMG') {
                if (v == 'full') return;
                if (rm) {
                    if (v == 'center') dom.setStyle(n.parentNode, 'textAlign', '');
                    dom.setStyle(n, 'float', '');
                    this.mceRepaint();
                    return;
                }
                if (v == 'center') {
                    if (/^(TD|TH)$/.test(bl.nodeName)) bl = 0;
        ...
        mceSetStyleInfo: function (u, v) {
            ...
            re = ed.settings.merge_styles_invalid_parents;
            if (tinymce.is(re, 'string')) re = new RegExp(re, 'i');
            if (e = t.getSelectedElement()) set(e, 1);
            else {
        ...
        FormatBlock: function (ui, val) {
            ...
            if (/^(P|DIV|H[1-6]|ADDRESS|BLOCKQUOTE|PRE)$/.test(ed.selection.getNode().nodeName)) t.mceRemoveNode();
        ...
        queryStateOutdent: function () {
            ...
            } else return !!ed.dom.getParent(ed.selection.getNode(), 'BLOCKQUOTE');
...
tinymce.create('tinymce.UndoManager', {
    ...
    add: function (l) {
        ...
        if (t.index < t.data.length && t.data[t.index].initial) t.index++;
    ...
    tinymce.create('tinymce.ForceBlocks', {
        ...
        forceRoots: function (ed, e) {
            var t = this,
                ...
                r = se.getRng(),
                si = -2,
                ei, so, eo, tr, c = -0xFFFFFF;
            var nx, bl, bp, sp, le, nl = b.childNodes,
                i;
        ...
        insertPara: function (e) {
            var t = this,
                ...
                r = s.getRangeAt(0),
                b = d.body;
            var rb, ra, dir, sn, so, en, eo, sb, eb, bn, bef, aft, sc, ec, n, vp = dom.getViewPort(ed.getWin()),
                y, ch;
            ...
            if (!sc.previousSibling && sc.parentNode.nodeName == bn) {
                ...
                if (rb.startContainer.nodeName == bn && rb.startOffset == 0) r.setStartBefore(rb.startContainer);
                else r.setStart(rb.startContainer, rb.startOffset);
            }
            if (!ec.nextSibling && ec.parentNode.nodeName == bn) r.setEndAfter(ec.parentNode);
            ...
            if (isEmpty(aft)) aft.innerHTML = isOpera ? '&nbsp;' : '<br />';
            if (isOpera) {
                r.insertNode(bef);
                r.insertNode(aft);
            } else {
        ...
        createDropMenu: function (id, s, cc) {
            ...
            if (tinymce.isIE) {
                c.onShowMenu.add(function () {
                    var s = ed.selection,
                        n = s.getNode();
                    if (n.nodeName == 'IMG') bm = s.getBookmark();
                    else bm = 0;
                });
...
(function () {
    ...
    tinymce.create('tinymce.WindowManager', {
        ...
        open: function (s, p) {
            var t = this,
                f = '',
                x, y, mo = t.editor.settings.dialog_type == 'modal',
                w, sw, sh, vp = tinymce.DOM.getViewPort(),
                u;
            s = s || {};
            p = p || {};
            ...
            if (mo) {
                ...
                } else s.modal = s.alwaysRaised = s.dialog = s.centerscreen = s.dependent = true;
            }
            each(s, function (v, k) {
