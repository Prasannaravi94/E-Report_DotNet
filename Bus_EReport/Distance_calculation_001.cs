...
namespace Bus_EReport
{
    public class Distance_calculation_001
    {
        ...
        public DataTable getHeadRecord(string month, string year, string sfCode, string dt)
        {
            ...
            strQry = "select sndhqfl Status, second_level_mgr level2_code, First_level_mgr level1_code from Trans_Expense_Head where Expense_Month=" + month + " and Expense_Year=" + year + " and sf_code='" + sfCode + "' and sf_joining_date='" + dt + "'";
            try
            {
        ...
        public DataTable getFieldForce(string divcode, string sfcode)
        {
            ...
            DataTable dsSubDiv = null;
            strQry = "select sf_code,sf_name+' - '+sf_Designation_Short_Name+' - '+sf_hq sf_name,sf_hq,sf_type,sf_emp_id Employee_Id,Employee_Id SF_Emp_Id,Division_Name,sf_desgn,convert(varchar(10),Sf_Joining_Date,101) Sf_Joining_Date,convert(varchar(10),Sf_Joining_Date,103) doj,Fieldforce_Type,D.Designation_Code,reporting_to_sf,S.state_code " +
                " FROM mas_salesforce S inner join Mas_SF_Designation D on D.Designation_Code=S.Designation_Code inner join Mas_Division MD on D.Division_Code=MD.Division_Code WHERE sf_code='" + sfcode + "'" +
                " ORDER BY sf_name";
            try
            {
                dsSubDiv = db_ER.Exec_DataTable(strQry);
        ...
        public DataTable getDist(string divCode, string sf_code)
        {
            ...
            DataTable dist = null;
            //strQry = "Select From_Code FrmTown,To_Code ToTown,Distance_In_Kms Distance,sf_code from Mas_Distance_Fixation where sf_Code in(select sf_code from mas_salesforce where reporting_To_sf='" + sf_code + "') and division_code='" + divCode + "'";
            // strQry = "select distinct From_Code FrmTown,alias_name+'-'+D.sf_code ToTown,Distance_In_Kms Distance,D.sf_code from Mas_Distance_Fixation D inner join mas_territory_creation T on D.to_code=cast(T.territory_code as varchar) and Territory_Active_Flag=0 inner join mas_salesforce S on T.sf_Code=S.sf_Code and S.sf_Code=D.from_code and reporting_to_sf='" + sf_code + "' order by D.sf_code";

            //            strQry = "select distinct From_Code FrmTown,alias_name+'-'+D.sf_code ToTown,Distance_In_Kms Distance,D.sf_code from Mas_Distance_Fixation D inner join mas_territory_creation T on D.to_code=cast(T.territory_code as varchar) and Territory_Active_Flag=0 inner join mas_salesforce S on T.sf_Code=S.sf_Code and S.sf_Code=D.from_code and reporting_to_sf='" + sf_code + "' inner join Mgr_Allowance_Setup SP on SP.sf_code=S.sf_Code and SP.mgr_code='" + sf_code + "' and Type_code='ACT' " +
            //"union all " +
            //"select distinct From_Code FrmTown,alias_name+'-'+S.sf_Code ToTown,Distance_In_Kms Distance,S.sf_Code sf_code from Mas_Distance_Fixation D inner join mas_territory_creation T on D.to_code=cast(T.territory_code as varchar) and Territory_Active_Flag=0 and D.sf_code='" + sf_code + "' inner join mas_salesforce S on T.sf_Code=S.sf_Code and reporting_to_sf='" + sf_code + "' inner join Mgr_Allowance_Setup SP on SP.sf_code=S.sf_Code and type_code='OS' and mgr_code='" + sf_code + "' and Territory_Cat!=1 " +
            //"union all " +
            //"select distinct From_Code FrmTown,alias_name+'-'+S.sf_Code ToTown,Distance_In_Kms Distance,S.sf_Code sf_code from Mas_Distance_Fixation D inner join mas_Salesforce s on S.sf_code=D.To_code and reporting_to_sf='" + sf_code + "' inner join mas_territory_creation T on T.sf_code=s.sf_code inner join Mgr_Allowance_Setup SP on SP.sf_code=S.sf_Code and SP.mgr_code='" + sf_code + "' and type_code='OS' and Territory_Cat=1 where D.sf_code='" + sf_code + "' and from_code='" + sf_code + "' order by D.sf_Code";
            strQry = "select distinct From_Code FrmTown,alias_name+'-'+D.sf_code ToTown,Distance_In_Kms Distance,D.sf_code from Mas_Distance_Fixation D inner join mas_territory_creation T on D.to_code=cast(T.territory_code as varchar) and Territory_Active_Flag=0 and T.division_code='" + divCode + "' inner join mas_salesforce S on T.sf_Code=S.sf_Code and S.sf_Code=D.from_code and reporting_to_sf='" + sf_code + "' inner join Mgr_Allowance_Setup SP on SP.sf_code=S.sf_Code and SP.mgr_code='" + sf_code + "' and Type_code='ACT' " +
           "union all " +
           "select distinct From_Code FrmTown,alias_name+'-'+D.sf_code ToTown,Distance_In_Kms Distance,D.sf_code from Mas_Distance_Fixation D inner join mas_territory_creation T on D.to_code=cast(T.territory_code as varchar) and Territory_Active_Flag=0 and T.division_code='" + divCode + "' and territory_Cat!=1 inner join mas_salesforce S on T.sf_Code=S.sf_Code and (S.reporting_to_sf=D.from_code or S.reporting_to_sf=D.to_code) and reporting_to_sf='" + sf_code + "' inner join Mgr_Allowance_Setup SP on SP.sf_code=S.sf_Code and SP.mgr_code='" + sf_code + "' and Type_code='OS' " +
           "union all " +
            "select DISTINCT From_Code FrmTown,alias_name+'-'+T.sf_code ToTown,Distance_In_Kms Distance,D.sf_code from mas_territory_creation T inner join mas_Salesforce S on T.sf_code=S.sf_code and T.division_code='"+ divCode + "' " +
          "inner join mas_distance_fixation D on D.sf_code='" + sf_code + "' and from_code='" + sf_code + "' and to_code=S.sf_code and to_code=T.sf_code " +
          "inner join Mgr_Allowance_Setup SP ON SP.sf_code=S.sf_Code and SP.mgr_code='" + sf_code + "' and Type_code='OS' " +
          "where S.reporting_to_sf='" + sf_code + "' and territory_cat=1 " +
            "order by D.sf_Code";
            try
            {
        ...
        public DataTable getotherWorkTypeMGR(string divcode)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataTable dsSubDiv = null;
            strQry = "SELECT Work_Type_Name,Allow_type,Work_Type_Code " +
                     " FROM Worktype_Allowance_Setup WHERE Type='2' and Division_Code like '%" + divcode + "%'" +
                     " ORDER BY Work_Type_Name";
            try
            {
                dsSubDiv = db_ER.Exec_DataTable(strQry);
        ...
        public DataTable getotherWorkTypeMGR(string divcode, string desig)
        {
            DB_EReporting db_ER = new DB_EReporting();
            ...
            {
                strQry = "SELECT Work_Type_Name,Allow_type,Work_Type_Code " +
                         " FROM Worktype_Allowance_Setup_Designation WHERE Type='2' and Desig_code='" + desig + "' and Division_Code like '%" + divcode + "%'" +
                         " ORDER BY Work_Type_Name";
            }
            try
            {
        ...
        public DataTable getFrmandTo(string divcode, string sf_code)
        {
            ...
            //strQry = "select territory_code,territory_name+'-'+ case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end +' ( '+sf_name +')' territory_name,case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end Town_Cat,S.sf_code " +
            //         "from mas_territory_creation T inner join mas_salesforce S on T.sf_code=S.sf_code where reporting_to_sf='" + sf_code + "' and T.division_Code='" + divcode + "'" +
            //          "order by sf_name,Territory_Cat,territory_name";
            //strQry = "select distinct Alias_name+'-'+S.sf_code territory_code,SP.type_code,alias_name+'-'+ case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end +' ( '+sf_name +')' territory_name,case when SP.type_code='OS' then 'OS' else case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end end Town_Cat,S.sf_code,S.sf_name " +
            //         "from mas_territory_creation T inner join mas_salesforce S on T.sf_code=S.sf_code and Territory_Active_Flag=0 inner join Mgr_Allowance_Setup SP on T.sf_code=SP.sf_code and mgr_code='" + sf_code + "' where reporting_to_sf='" + sf_code + "' and T.division_Code='" + divcode + "' " +
            //          "order by sf_name,Town_Cat,territory_name";
            strQry = "select distinct Alias_name+'-'+S.sf_code territory_code,SP.type_code,alias_name+'-'+ case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end +' ( '+sf_name +')' territory_name,case when SP.type_code='OS' then 'OS' else case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end end Town_Cat,case when SP.type_code='OS' then 'OS' else case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end end Town_Cat,case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end town_Cat1,S.sf_code,S.sf_name,Territory_Cat " +
                     "from mas_territory_creation T inner join mas_salesforce S on T.sf_code=S.sf_code and Territory_Active_Flag=0 inner join Mgr_Allowance_Setup SP on T.sf_code=SP.sf_code and mgr_code='" + sf_code + "' where reporting_to_sf='" + sf_code + "' and T.division_Code='" + divcode + "' and type_code!='OS' " +
                     "Union All " +
                     "select distinct Alias_name+'-'+S.Reporting_to_sf territory_code,SP.type_code,alias_name+'-'+ case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end +' ( '+sf_name +')' territory_name,case when SP.type_code='OS' then 'OS' else case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end end Town_Cat,case when SP.type_code='OS' then 'OS' else case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end end Town_Cat,case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end town_Cat1,S.sf_code,S.sf_name,Territory_Cat " +
                     "from mas_territory_creation T inner join mas_salesforce S on T.sf_code=S.sf_code and Territory_Active_Flag=0 and territory_Cat!=1 inner join Mgr_Allowance_Setup SP on T.sf_code=SP.sf_code and mgr_code='" + sf_code + "' where reporting_to_sf='" + sf_code + "' and T.division_Code='" + divcode + "' and type_code='OS' " +
                     "Union All " +
                     "select distinct Alias_name+'-'+S.sf_code territory_code,SP.type_code,alias_name+'-'+ case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end +' ( '+sf_name +')' territory_name,case when SP.type_code='OS' then 'OS' else case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end end Town_Cat,case when SP.type_code='OS' then 'OS' else case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end end Town_Cat,case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end town_Cat1,S.sf_code,S.sf_name,Territory_Cat " +
                     "from mas_territory_creation T inner join mas_salesforce S on T.sf_code=S.sf_code and Territory_Active_Flag=0 and territory_Cat=1 inner join Mgr_Allowance_Setup SP on T.sf_code=SP.sf_code and mgr_code='" + sf_code + "' where reporting_to_sf='" + sf_code + "' and T.division_Code='" + divcode + "' and type_code='OS' " +
                      "order by sf_name,Territory_Cat,territory_name";
        ...
        public DataTable getFrmandToMGR(string divcode, string sf_code)
        {
            DB_EReporting db_ER = new DB_EReporting();
            ...
            strQry = "select distinct Alias_name+'-'+S.sf_code territory_code,SP.type_code,alias_name+'-'+ case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end +' ( '+sf_name +')' territory_name,case when SP.type_code='OS' then 'OS' else case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end end Town_Cat,case when SP.type_code='OS' then 'OS' else case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end end Town_Cat,case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end town_Cat1,S.sf_code,S.sf_name,Territory_Cat " +
                     "from mas_territory_creation T inner join mas_salesforce S on T.sf_code=S.sf_code and Territory_Active_Flag=0 inner join Mgr_Allowance_Setup SP on T.sf_code=SP.sf_code and mgr_code='" + sf_code + "' where reporting_to_sf IN(SELECT sf_code from mas_salesforce where reporting_to_sf='" + sf_code + "') and T.division_Code='" + divcode + "' and type_code!='OS' " +
                     "Union All " +
                     "select distinct Alias_name+'-'+S.Reporting_to_sf territory_code,SP.type_code,alias_name+'-'+ case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end +' ( '+sf_name +')' territory_name,case when SP.type_code='OS' then 'OS' else case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end end Town_Cat,case when SP.type_code='OS' then 'OS' else case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end end Town_Cat,case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end town_Cat1,S.sf_code,S.sf_name,Territory_Cat " +
                     "from mas_territory_creation T inner join mas_salesforce S on T.sf_code=S.sf_code and Territory_Active_Flag=0 and territory_Cat!=1 inner join Mgr_Allowance_Setup SP on T.sf_code=SP.sf_code and mgr_code='" + sf_code + "' where reporting_to_sf IN(SELECT sf_code from mas_salesforce where reporting_to_sf='" + sf_code + "') and T.division_Code='" + divcode + "' and type_code='OS' " +
                     "Union All " +
                     "select distinct Alias_name+'-'+S.sf_code territory_code,SP.type_code,alias_name+'-'+ case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end +' ( '+sf_name +')' territory_name,case when SP.type_code='OS' then 'OS' else case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end end Town_Cat,case when SP.type_code='OS' then 'OS' else case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end end Town_Cat,case when Territory_Cat=1 then 'HQ' else case when Territory_Cat=2 then 'EX' else case when Territory_Cat=3 then 'OS' else 'OS-EX' end end end town_Cat1,S.sf_code,S.sf_name,Territory_Cat " +
                     "from mas_territory_creation T inner join mas_salesforce S on T.sf_code=S.sf_code and Territory_Active_Flag=0 and territory_Cat=1 inner join Mgr_Allowance_Setup SP on T.sf_code=SP.sf_code and mgr_code='" + sf_code + "' where reporting_to_sf IN(SELECT sf_code from mas_salesforce where reporting_to_sf='" + sf_code + "') and T.division_Code='" + divcode + "' and type_code='OS' " +
                      "order by sf_name,Territory_Cat,territory_name";
        ...
        public DataSet getExDistance(string divcode, string sf_code, string mnth, string yr)
        {
            DB_EReporting db_ER = new DB_EReporting();
            DataSet dsDivision = null;
            strQry = "select max(isnull(Distance_in_kms,0)*2)dist,activity_date adate from DCRDetail_Lst_Trans D inner join DCRMain_Trans H on D.Trans_SlNo=H.Trans_SlNo " +
           "inner join Mas_ListedDr L on L.listeddrcode=D.Trans_Detail_Info_Code " +
          "inner join Mas_Territory_Creation T on CAST(ltrim(T.Territory_Code) as varchar)=L.Territory_Code and L.sf_code=T.sf_code " +
           "inner join  mas_salesforce sf on sf.sf_code=T.sf_code " +
           "inner join Mas_Distance_Fixation DS on DS.To_Code in(select CAST(ltrim(Territory_Code) as varchar) from mas_territory_creation where sf_code=T.sf_Code and alias_name=T.alias_name) and from_code=T.sf_code " +
       "where H.Sf_Code='" + sf_code + "' and D.Division_Code='" + divcode + "' and MONTH(Activity_Date)='" + mnth + "' " +
      "and YEAR(Activity_Date)='" + yr + "' group by activity_date";
            try
            {
                dsDivision = db_ER.Exec_DataSet(strQry);
            }
        ...
        public DataTable getSavedRecord(string month, string year, string sf_code, string dt)
        {
            ...
            {
                DB_EReporting db = new DB_EReporting();
                strQry = "SELECT catTemp,temtyp,Expense_Date Adate, adate1, Expense_Day theDayName ,Expense_wtype Worktype_Name_B,Expense_wtype Worktype_Name_M,Place_of_Work Territory_Name,Place_of_Work TerrPlaces,Expense_All_Type Territory_Cat,Expense_All_Type Type_Code,Expense_Allowance Allowance,Expense_Distance Distance,Expense_Fare Fare,Expense_Total Total,D.Division_Code,from_place,to_place,mgr_amount,mgr_remarks,mgr_total,isnull(mgr_amount2,'') mgr_amount2,mgr_remarks2,isnull(mgr_total2,'') mgr_total2,confirm_amount,confirm_remarks,confirm_total,Mgr_Allow,isnull(Mgr_Allow2,'') Mgr_Allow2,confirm_allow,calc_flag,calc_flag2,exp_remarks,frmTovalues,isnull(rw_amount,0)rw_amount,rw_rmks,Local_convey,Bill_Type,Total_allowance from Trans_Expense_detail D" +
        ...
        public DataTable getSavedOtheExpRecord(string month, string year, string sf_code, string dt)
        {
            ...
            try
            {
                ...
                strQry = "SELECT F.Expense_Parameter_Name Paritulars,amt,other_total,remarks,expval from Exp_others O inner join Fixed_Variable_Expense_Setup F on SUBSTRING(O.expval,0, CHARINDEX('##',O.expval))=F.Expense_Parameter_Code " +
                         "where sl_no in(SELECT sl_No FROM Trans_Expense_Head WHERE SF_Code='" + sf_code + "' and Expense_Month=" + month + " and Expense_Year=" + year + ") ";
                data = db.Exec_DataTable(strQry);
            }
            catch (Exception ex)
        ...
        public int getValidheadRecordNo(string month, string year, string sf_code, string dt1)
        {
            ...
            try
            {
                ...
                strQry = "SELECT sl_No ,sndhqfl status FROM Trans_Expense_Head WHERE SF_Code='" + sf_code + "' and Expense_Month=" + month + " and Expense_Year=" + year + " and sf_joining_date='" + dt1 + "'";
        ...
        public int addHeadRecord(Dictionary<String, String> values)
        {
            ...
            try
            {
                ...
                if (headRecordExist(values["month"], values["year"], values["sf_code"], values["soj"]))
                {
                    ...
                    deleteFixed(values);
                    strQry = "delete from exp_accinf WHERE sl_no in(SELECT sl_No FROM Trans_Expense_Head WHERE SF_Code='" + values["sf_code"] + "' and Expense_Month=" + values["month"] + " and Expense_Year=" + values["year"] + " and sf_joining_date='" + values["soj"] + "')";
                    iReturn = db.ExecQry(strQry);
                    strQry = "delete from Trans_Expense_detail " +
        ...
        private int insertHeadRecord(Dictionary<String, String> values, int iReturn, DB_EReporting db, int mgrFlg)
        {
            ...
            strQry = "INSERT INTO Trans_Expense_Head " +
                "(Sl_No,SF_Code,Expense_Month,Expense_Year,Division_Code,sndhqfl,submission_date,Expense_Period,mgr_flag,Status_flg,Employee_Id,sf_emp_id,frmTovalues,sf_joining_date,file_path)" +
                "VALUES (" + sNo + ",'" +
                values["sf_code"] + "'," + values["month"] + "," + values["year"] + "," + values["div_code"] + "," + values["flag"] + ",getdate()," + values["flag"] + "," + mgrFlg + ",0,'" + values["EmpNo"] + "','" + values["SfEmpNo"] + "','" + values["frmTovalues"] + "','" + values["soj"] + "','" + values["File_Path"] + "')";
            iReturn = db.ExecQry(strQry);
            return sNo;
        }
        ...
        public DataSet sfcviewnew(string sf_code)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsSF = null;

            strQry = "EXEC sfcviewnew '" + sf_code + "'";
        ...
        public DataSet sfcwrktyp(string sf_code)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsSF = null;

            strQry = "select SF.sf_Code,sf_name,type_code,Type_Name, SF.Sf_HQ from mgr_allowance_Setup S inner join mas_salesforce SF on S.sf_Code=SF.sf_code where mgr_code='" + sf_code + "'";
        ...
        public DataSet sfcviewnew_MGR(string sf_code)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsSF = null;

            strQry = "EXEC sfcviewnew_MGR '" + sf_code + "'";
...
"where month(activity_date)='" + mnth + "' and year(activity_date)='" + yr + "' and H.sf_code='" + sf_code + "')as d)as terr group by activity_date order by activity_date";
        ...
        public DataSet getExpense_MGR_prevnextrelation_All_Dates(string mnth, string yr)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsDivision = null;
            strQry = "exec getExpense_MGR_prevnextrelation_All_Dates '" + mnth + "','" + yr + "'";
            try
            {
                dsDivision = db_ER.Exec_DataSet(strQry);
            }
        ...
        public DataSet getExpense_MGR_prevnextrelation_vacant(string sf_code, string mnth, string yr, string stdt, string enddt)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsDivision = null;
            strQry = "exec getExpense_MGR_prevnextrelation_vacant '" + sf_code + "', '" + mnth + "','" + yr + "','" + stdt + "','" + enddt + "'";
            try
            {
                dsDivision = db_ER.Exec_DataSet(strQry);
            }
        ...
        public DataSet getMGRAllowTyp_CA(string sf_code, string mnth, string yr)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsPlace = null;
            strQry = "exec mgrallowtype_CA '" + sf_code + "' ,'" + mnth + "','" + yr + "'";
            try
            {
                dsPlace = db_ER.Exec_DataSet(strQry);
            }
        ...
        public DataSet MGR_Placeofwork_CA(string sf_code, string mnth, string yr)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsPlace = null;
            strQry = "exec MGR_Placeofwork_CA '" + sf_code + "' ,'" + mnth + "','" + yr + "'";
            try
            {
                dsPlace = db_ER.Exec_DataSet(strQry);
            }
        ...
        public DataSet getPlaceMGRNEW(string divcode,string sf_code, string mnth, string yr)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsPlace = null;
            strQry = "exec getExpense_MGR_Places '" + sf_code + "' ,'" + mnth + "','" + yr + "','"+divcode+"'";
            try
            {
                dsPlace = db_ER.Exec_DataSet(strQry);
            }
        ...
        public DataTable sp_Get_Fixed_expense_MGR(String divcode, string sf_code, string mnth, string yr)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataTable dsDivision = null;

            strQry = "exec sp_Get_Fixed_expense_MGR '" + divcode + "','" + sf_code + "','" + mnth + "','" + yr + "'";
            try
            {
                dsDivision = db_ER.Exec_DataTable(strQry);
            }
        ...
        public DataSet getPlaceMGRNEW_cibles(string divcode, string sf_code, string mnth, string yr)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsPlace = null;
            strQry = "exec getExpense_MGR_Places_Cibles '" + sf_code + "' ,'" + mnth + "','" + yr + "','" + divcode + "'";
            try
            {
                dsPlace = db_ER.Exec_DataSet(strQry);
            }
        ...
        public DataSet getExpense_MGR_cibles(string sf_code, string mnth, string yr, string divcode)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsDivision = null;
            strQry = "exec getExpense_MGR_cibles '" + sf_code + "', '" + mnth + "','" + yr + "','" + divcode + "'";
            try
            {
                dsDivision = db_ER.Exec_DataSet(strQry);
            }
        ...
        public DataTable getFieldForce_new(string divcode, string sfcode)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataTable dsSubDiv = null;
            strQry = "select sf_code,sf_name,sf_Designation_Short_Name,sf_hq,sf_type,sf_emp_id Employee_Id,Employee_Id SF_Emp_Id,Division_Name,sf_desgn,convert(varchar(10),Sf_Joining_Date,101) Sf_Joining_Date,convert(varchar(10),Sf_Joining_Date,103) doj,Fieldforce_Type " +
                " FROM mas_salesforce S inner join Mas_SF_Designation D on D.Designation_Code=S.Designation_Code inner join Mas_Division MD on D.Division_Code=MD.Division_Code WHERE sf_code='" + sfcode + "'" +
                " ORDER BY sf_name";
            try
            {
