...
function dayplan() {
	...
	$InsMode=(string) $data['InsMode'];
		   ...
		   $query="select Work_Type,WorkType_Name,isnull(FWFlg,'F') FWFlg,Half_Day_FW,Typ from vwActivity_Report where Sf_Code='".$data['sfcode']."' and cast(convert(varchar,Activity_Date,101) as datetime)=cast(convert(varchar,cast ('".$data['TPDt']."' as datetime),101) as datetime) and Work_Type<>'".$data['WT_code']."'";
