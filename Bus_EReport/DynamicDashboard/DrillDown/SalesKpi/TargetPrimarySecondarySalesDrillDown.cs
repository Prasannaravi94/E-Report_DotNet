...
namespace Bus_EReport.DynamicDashboard.DrillDown.SalesKpi
{
    ...
    public class TargetPrimarySecondarySalesDrillDown : DrilldownBase
    {
        ...
        public TargetPrimarySecondarySalesDrillDown()
        {
            ...
            Columns = new Dictionary<string, DrillDownColumn>
            {
                ...
                {"PrimarySalesQuantity", new DrillDownColumn {Label = "Primary Sales Quantity", Select = "0 as PrimarySalesQuantity"}},
                {"SecondarySalesQuantity", new DrillDownColumn {Label = "Secondary Sales Quantity", Select = "0 as SecondarySalesQuantity"}},
                ...
                {"PrimarySalesValue", new DrillDownColumn {Label = "Primary Sales Value", Select = "0 as PrimarySalesValue"}},
                {"SecondarySalesValue", new DrillDownColumn {Label = "Secondary Sales Value", Select = "0 as SecondarySalesValue"}},
            };
            ...
            VisibleColumns = new List<string>
            {
                ...
                "Month",
                "TargetQuantity",
                "PrimarySalesQuantity",
                "SecondarySalesQuantity",
                "TargetValue",
                "PrimarySalesValue",
                "SecondarySalesValue",
            };
        ...
        {
            FileName = "T Vs P Vs S Sales";
            PreferenceName = "TargetPrimarySecondarySalesDrillDown";
        ...
        {
            var strQry = "    SELECT " +
                "Trans_SS_Entry_Detail.Product_Detail_Code AS ProductId," +
                "Trans_SS_Entry_Detail_value.Sec_Sale_Qty AS Sale_Quantity," +
                "CAST(Trans_SS_Entry_Detail_value.Sec_Sale_Qty AS float) * Trans_SS_Entry_Detail." + SecsalesValueColumn + " as Sale_Value, " +
                "FORMAT(CAST(CONCAT(CAST(Month AS VARCHAR(2)), '-01-', CAST(Year AS VARCHAR(4))) AS DATE), 'MMM yyyy') AS Month," +
        ...
        public string getQuery(bool count = false)
        {
            ...
            else
            {
                ...
                if (VisibleColumns.Contains("SecondarySalesQuantity"))
                {
                    strQry += ",SUM(SecondarySales.Sale_Quantity) AS SecondarySalesQuantity";
                }
                if (VisibleColumns.Contains("TargetValue"))
                {
                    strQry += ",SUM(Targets.TargetQuantity)*SUM(Targets.TargetValue) AS TargetValue";
                }
                if (VisibleColumns.Contains("SecondarySalesValue"))
                {
                    strQry += ",SUM(SecondarySales.Sale_Value) AS SecondarySalesValue";
                }
                
                if (VisibleColumns.Contains("PrimarySalesQuantity"))
                {
                    strQry += ",SUM(PrimaryBill.Sale_Quantity) AS PrimarySalesQuantity";
                }
                if (VisibleColumns.Contains("PrimarySalesValue"))
                {
                    strQry += ",SUM(PrimaryBill.Sale_Value) AS PrimarySalesValue";
                }
            }
            strQry += " FROM Targets " +
                "LEFT JOIN SecondarySales ON Targets.ProductId = SecondarySales.ProductId AND SecondarySales.HQ_Code=Targets.HQCode AND SecondarySales.Month=Targets.Month "+
                "LEFT JOIN PrimaryBill ON Targets.ProductId = PrimaryBill.ProductId AND PrimaryBill.HQ_Code=Targets.HQCode AND PrimaryBill.Month=Targets.Month ";
