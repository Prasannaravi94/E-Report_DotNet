	...
	for ($Ri = 0; $Ri < count($RCPADatas); $Ri++) 
    {
        ...
        {   
          ...
          // {
              ...
              // $arr = performQuery($query);
              // if(count($arr)>0){
                 // $query="exec ios_svDCRCSHDet_App_Edet '".$ARNo."',0,'".$data['sfcode']."','2','".$Chms[$Rj]["Code"]."','".$Chms[$Rj]["Name"]."','".$arr[0]["tmv"]."',0,'".$arr[0]["Worked_with_Code"]."','','','','','','','".$arr[0]["Division_code"]."',0,'".$arr[0]["tmv"]."','".$arr[0]["lati"]."','".$arr[0]["long"]."','".$arr[0]["DataSF"]."','NA','".$data['Mod']."','".$data['town_code']."','".$data['town_name']."'";
    	          // $result["CQry"]=$query;
      	          // performQuery($query);
              // }
          // }
