...
'dayno' =>$res[$il]["dayno"]));
		...
		array_push($result,array('SFCode' => $sfCode,'SFName' =>$res[0]["SFName"],'DivCode' =>$res[0]["Div"],'status' =>$res[0]["Change_Status"],'TPDatas' =>$resTP,'TPFlag' => '0','TPMonth' => $res[0]["Mnth"],'TPYear' => $res[0]["Yr"],'Reject_reason' => $res[0]["Rejection_Reason"]));
    }
    return $result;
}
    ...
    $ExisArr = performQuery($query);
	  if ($ExisArr[0]["Cnt"]>0){
		...
		if ($ExisArr[0]["Cnt"]>0 && $InsMode=="0"){
			...
			$query="exec iOS_svTodayTP '".$sfCode."','".$SFMem."','".$PlnCd."','".$PlnNM."','".$WT."','".$WTNM."','".$Rem."','".$loc."','".$TPDt."','".$TpVwFlg."'";
			performQuery($query);
			if ($InsMode=="2")
			{
...
//echo $sfCode;
	...
	for ($i = 0; $i < count($TPDatas); $i++) 
    {
		...
		if($TPData["dayno"]!="")
		{
            ...
            $TPJWCd=array();$TPJWNm=array();$TPRmks=array();$TPSTCd=array();$TPSTNm=array();
			for($il=0;$il<count($TPDet);$il++){
...
//print_r($TPData["DayPlan"]);
		...
		if($TPData["dayno"]!="")
		{
			$TPDet=$TPData["DayPlan"]; 
			$TPWTCd=array();$TPWTNm=array();$TPSFCd=array();$TPSFNm=array();$TPPlCd=array();$TPPlNm=array();
			$TPDRCd=array();$TPDRNm=array();$TPCHCd=array();$TPCHNm=array();$TPJWCd=array();$TPJWNm=array();$TPRmks=array();$TPSTCd=array();$TPSTNm=array();
//print_r($TPDet);
			for($il=0;$il<count($TPDet);$il++){
//print_r($TPDet[$il]["WTCd"]);
				...
				array_push($TPCHCd,$TPDet[$il]["Chem_Code"]);
				array_push($TPCHNm,$TPDet[$il]["Chem_Name"]);
				array_push($TPSTCd,$TPDet[$il]["Stck_Code"]);
                array_push($TPSTNm,$TPDet[$il]["Stck_Name"]);
				array_push($TPRmks,$TPDet[$il]["DayRemarks"]);

			}

			$query="exec iOS_svTourPlanNew '".$sfCode."','".$sfName."','".$data[0]['TPMonth']."','".$data[0]['TPYear']."','".$TPData["TPDt"]."','".$Stat."','".$TPWTCd[0]."','".$TPWTCd[1]."','".$TPWTCd[2]."','".$TPWTNm[0]."','".$TPWTNm[1]."','".$TPWTNm[2]."','".$TPSFCd[0]."','".$TPSFCd[1]."','".$TPSFCd[2]."','".$TPSFNm[0]."','".$TPSFNm[1]."','".$TPSFNm[2]."','".$TPPlCd[0]."','".$TPPlCd[1]."','".$TPPlCd[2]."','".$TPPlNm[0]."','".$TPPlNm[1]."','".$TPPlNm[2]."','".$TPJWCd[0]."','".$TPJWCd[1]."','".$TPJWCd[2]."','".$TPJWNm[0]."','".$TPJWNm[1]."','".$TPJWNm[2]."','".$TPDRCd[0]."','".$TPDRCd[1]."','".$TPDRCd[2]."','".$TPDRNm[0]."','".$TPDRNm[1]."','".$TPDRNm[2]."','".$TPCHCd[0]."','".$TPCHCd[1]."','".$TPCHCd[2]."','".$TPCHNm[0]."','".$TPCHNm[1]."','".$TPCHNm[2]."','".$TPRmks[0]."','".$TPRmks[1]."','".$TPRmks[2]."','".$DivCode[0]."','".$TPSTCd[0]."','".$TPSTNm[0]."','".$TPSTCd[1]."','".$TPSTNm[1]."','".$TPSTCd[2]."','".$TPSTNm[2]."'";
  //		echo $query;
	performQuery($query);
			$result["Qry"]=$query;
		} 
...
$vst="0";
            ...
            if ($vTyp != 0) {
                ...
                if ($vTyp == 1 || $vTyp == 4|| $vTyp == 6)
                    $sqlsp = $sqlsp . "?,?,?,?,?,";
               	if ($vTyp == 1|| $vTyp == 6)   {
				  $sqlsp = $sqlsp . "'" . str_replace("'", "", $vals["Town_code"]) . "','" . str_replace("'", "", $vals["Daywise_Remarks"]) . "',?,'" . str_replace("'", "", $vals["rx_t"]) . "','" . date('Y-m-d H:i:s') . "',?,?,'" . str_replace("'", "", $vals["DataSF"]) . "','" . $DetTB["geoaddress"] . "',".$apps.",'" . str_replace("'", "", $nextVisitDate) . "')}";

				} 
				else
				 $sqlsp = $sqlsp . "'" . str_replace("'", "", $vals["Town_code"]) . "','" . str_replace("'", "", $vals["Daywise_Remarks"]) . "',?,'" . str_replace("'", "", $vals["rx_t"]) . "','" . date('Y-m-d H:i:s') . "',?,?,'" . str_replace("'", "", $vals["DataSF"]) . "','" . $DetTB["geoaddress"] . "')}";
