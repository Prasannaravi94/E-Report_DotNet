...
//Reactivation
        ...
        public DataSet FetchClass_Select(string sf_code)
        {
            ...
            strQry = " SELECT Doc_ClsCode,Doc_ClsName " +
                     " FROM  Mas_Doc_Class where division_Code = '" + div_code + "' AND doc_cls_activeflag=0 ";
            try
            {
                dsTerr = db_ER.Exec_DataSet(strQry);
        ...
        public DataSet getUnListDr_Allow(string div_code, string sf_code)
        {
            DB_EReporting db_ER = new DB_EReporting();
            DataSet dsListedDR = null;

            strQry = "Select Unlst_Cnt from Screen_Lock where Div_Code='" + div_code + "' and SF_Code ='" + sf_code + "'";
            try
            {
                dsListedDR = db_ER.Exec_DataSet(strQry);
            }
        ...
        public DataSet getUnListDr_Count(string sf_code, string div_code)
        {
            DB_EReporting db_ER = new DB_EReporting();
            DataSet dsListedDR = null;

            strQry = strQry = "SELECT COUNT(UnListedDrCode) FROM mas_unlisteddr WHERE Sf_Code = '" + sf_code + "' and Division_Code ='" + div_code + "' and (UnListedDr_Active_Flag = 0)";
        ...
        public int ReActivate(string dr_code)
        {
            ...
            try
            {
                ...
                strQry = "UPDATE Mas_UnListedDr " +
                            " SET Unlisteddr_active_flag=0 , " +
                            " Unlisteddr_deactivate_date = NULL " +
                            " WHERE Unlisteddrcode = '" + dr_code + "' ";
        ...
        public DataSet getSetup_Price(string div_code)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsSetup_Price = null;

            strQry = "select CRMBus_Cal_Based from Setup_Others where division_code='" + div_code + "' ";
            try
            {
                dsSetup_Price = db_ER.Exec_DataSet(strQry);
            }
            catch (Exception ex)
            {
                throw ex;
            }
            return dsSetup_Price;
        }
    }
}
