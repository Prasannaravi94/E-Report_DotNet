...
background: #FFEFD5; /* Old browsers */
    ...
    <script type="" language="javascript">
        ...
        function DRForPopup($x, $r, rCnt) {
            //            var $temp = $r.cells[1].childNodes[0].value.replace(/,/g, '');
            //          if (isNaN($temp) || $temp == '') $temp = 0;
            popupCal($r.cells[1].children[0], 1);
            var tb = $r.parentNode;
            var Ttb = tb.parentNode
            if (Ttb.rows.length - 1 > rCnt) {
                tb.removeChild($r);
        ...
        function popupCal($OthExpVal, flag) {
            var amt = parseFloat($OthExpVal.parentNode.parentNode.cells[2].children[0].value);
            var amtType = parseFloat($OthExpVal.parentNode.parentNode.cells[0].children[0].options[$OthExpVal.parentNode.parentNode.cells[0].children[0].selectedIndex].value);
            //alert(amt);
            ...
            for (var $rl = 1; $rl < $R.children.length; $rl++) {
                ...
                remarks = $R.children[$rl].cells[3].childNodes[0].value.replace(/,/g, '');
                paramName = $R.children[$rl].cells[1].children[0].options[$R.children[$rl].cells[1].children[0].selectedIndex].text;
                paramVal = $R.children[$rl].cells[1].children[0].options[$R.children[$rl].cells[1].children[0].selectedIndex].value;
                var value = $R.children[$rl].cells[0].children[0].options[$R.children[$rl].cells[0].children[0].selectedIndex].value;
                if (value == 2 && flag == 0) {
                ...
                if (value == 1) {
                    ...
                    if (addexp == "") {
                        addexp = "1~" + paramName + "=" + paramVal + "~" + $temp + "~" + remarks;
                    }
                    else {
                        addexp = addexp + "$1~" + paramName + "=" + paramVal + "~" + $temp + "~" + remarks;
                    }
                }
                if (value == 0) {
                    $Tot = parseFloat($Tot) - parseFloat($temp);
                    if (addexp == "") {
                        addexp = "0~" + paramName + "=" + paramVal + "~" + $temp + "~" + remarks;
                    }
                    else {
                        addexp = addexp + "$0~" + paramName + "=" + paramVal + "~" + $temp + "~" + remarks;
                    }
            ...
            //alert($Tot);
            var $P = $OthExpVal.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode;
            $P.cells[9].childNodes[2].value = addexp;
            var dat = $P.cells[0].innerText.substring(0, 2);
            if (isNaN(dat)) dat = 0;
            dat = parseInt(dat - 1);
            //alert(dat);
            var t = document.getElementsByName("popupTotalName");
            $popupTotE = t[dat];
            var popupTotVal = parseFloat($popupTotE.innerHTML.replace(/,/g, ''));
            //if (isNaN(popupTotVal)) popupTotVal = 0;
            if (flag == 1) {
                if (amtType == 1)
                    $popupTotE.innerHTML = $Tot - amt;
                else
                    $popupTotE.innerHTML = $Tot + amt;
            }
            else {
                $popupTotE.innerHTML = $Tot;
            }
...
<body class="bodycss">
   ...
   <td><asp:DropDownList ID="yearID" runat="server"></asp:DropDownList></td>
        <td><asp:Button ID="btnSubmit" runat ="server" Text ="Go" CssClass="savebutton"
                                ...
                                <Columns>
                                    ...
                                    <asp:TemplateField HeaderText="Place of Work" ItemStyle-HorizontalAlign="Left">
                                     ...
                                     <asp:TemplateField HeaderText="Distance Travelled</br>(in Kms)" ItemStyle-HorizontalAlign="Left">
                                        ...
                                        <HeaderStyle BorderStyle="Solid" BorderWidth="1px" BorderColor="#9AA3A9"></HeaderStyle>
                                        <ItemTemplate><asp:Label ID="lblDistance" runat="server" Text='<%# Bind("Distance") %>'></asp:Label><asp:HiddenField ID="distHidden" runat="server" Value='<%# Bind("Distance") %>' /></ItemTemplate></asp:TemplateField>
                                     <asp:TemplateField HeaderText="Fare</br>(in Rs/-)" ItemStyle-HorizontalAlign="Left">
                                        <ControlStyle></ControlStyle>
                                        <ItemStyle BorderStyle="Solid" BorderWidth="1px" BorderColor="#9AA3A9" HorizontalAlign="center">
                                        </ItemStyle>
                                        <HeaderStyle BorderStyle="Solid" BorderWidth="1px" BorderColor="#9AA3A9"></HeaderStyle>
                                        <ItemTemplate><asp:Label ID="lblFare" runat="server" Text='<%# Bind("Fare") %>'></asp:Label><asp:TextBox ID="txtFare" onkeyup="onFareChange(this);" visible="false" width="30" runat="server" Text='<%# Bind("Fare") %>'></asp:TextBox><asp:HiddenField ID="fareHidden" runat="server" Value='<%# Bind("Fare") %>' />
                                        </ItemTemplate>
                                    </asp:TemplateField>
                                    ...
                                    <HeaderStyle BorderStyle="Solid" BorderWidth="1px" BorderColor="#9AA3A9"></HeaderStyle>
                                    <ItemTemplate><asp:Label ID="lbladdexp" runat="server" Text='<%# Bind("rw_amount") %>' ></asp:Label><asp:HiddenField ID="addExpHidden" runat="server" Value='<%# Bind("rw_amount") %>' /></ItemTemplate>
                                </asp:TemplateField>
...
<script language="javascript">
    ...
    function _CalDist($x, TotDist, fare) {
        ...
        if ((idString.substring($i1, $i2) - 1) < 10) {
            //alert(idString);
            //alert(document.getElementById("grdExpMain_ctl" + (idString.substring($i1, $i2)) + "_lblWorkType").innerText);
            //alert(document.getElementById("grdExpMain_ctl0" + (idString.substring($i1, $i2) - 1) + "_lblWorkType").innerText);
            //alert(document.getElementById("grdExpMain_ctl" + (idString.substring($i1, $i2)) + "_toPlace").value);
            //if ((document.getElementById("grdExpMain_ctl" + (idString.substring($i1, $i2)) + "_lblWorkType").innerText) == (document.getElementById("grdExpMain_ctl0" + (idString.substring($i1, $i2) - 1) + "_lblWorkType").innerText)) {
            ...
            //prevValue = document.getElementById("grdExpMain_ctl0" + (idString.substring($i1, $i2) - 1) + "_toPlace").value;
            //alert("prev val "+ prevValue);
            currValue = document.getElementById("grdExpMain_ctl" + (idString.substring($i1, $i2)) + "_toPlace").value;
            //alert(document.getElementById("grdExpMain_ctl" + (idString.substring($i1, $i2)) + "_toPlace").id);
            //alert("curr val "+currValue);
            //alert("grdExpMain_ctl" + (parseInt(idString.substring($i1, $i2)) + 1));
            var cId = parseInt(idString.substring($i1, $i2)) + 1;
            //alert("grdExpMain_ctl" + cId + "_toPlace");
            //alert(document.getElementById("grdExpMain_ctl" + cId + "_toPlace").value);
            if (cId < 10) {
                // alert("grdExpMain_ctl0" + cId + "_toPlace");alert(document.getElementById("grdExpMain_ctl0" + cId + "_toPlace"));
            ...
            //alert("next val "+nextValue);
            //                PprevValue = document.getElementById("grdExpMain_ctl0" + (idString.substring($i1, $i2) - 2) + "_toPlace").value;
            //                alert(PprevValue);
            ...
            //            
            //alert(document.getElementById("grdExpMain_ctl" + (idString.substring($i1, $i2)) + "_lblWorkType").innerText);
            //alert(document.getElementById("grdExpMain_ctl" + (idString.substring($i1, $i2) - 1) + "_lblWorkType").innerText);
            //if ((document.getElementById("grdExpMain_ctl" + (idString.substring($i1, $i2)) + "_lblWorkType").innerText) == (document.getElementById("grdExpMain_ctl" + (idString.substring($i1, $i2) - 1) + "_lblWorkType").innerText)) {
            var cIdprev = parseInt(idString.substring($i1, $i2)) - 1;
            ...
            //prevValue = document.getElementById("grdExpMain_ctl" + (idString.substring($i1, $i2) - 1) + "_toPlace").value;
            currValue = document.getElementById("grdExpMain_ctl" + (idString.substring($i1, $i2)) + "_toPlace").value;
            var cId = parseInt(idString.substring($i1, $i2)) + 1;
            nextEl = document.getElementById("grdExpMain_ctl" + cId + "_toPlace");
            if (nextEl != "" && nextEl != null)
                nextValue = document.getElementById("grdExpMain_ctl" + cId + "_toPlace").value;
            // alert(nextValue);
            //                PprevValue = document.getElementById("grdExpMain_ctl" + (idString.substring($i1, $i2) - 2) + "_toPlace").value;
            //                alert(PprevValue);
            pretyp = prevValue.split('~~');
            pretyp1 = pretyp[1];
            curtyp = currValue.split('~~');
        ...
        //                            prevCurr = "EQ";
        //                            alert(prevValue + " prev and curre value equal " + currValue+" :: "+prevCurr);
        //                        }
        //                        else {
        //                            prevCurr = "NEQ";
        //                            alert(prevValue + " prev and curre value not equal " + currValue + " :: " + prevCurr);
        //                        }
        //                        if (nextValue == currValue) {
        //                            currNext = "EQ";
        //                            alert(nextValue + " next and curr value equal " + currValue + " :: " + currNext);
        //                        }
        //                        else {
        //                            currNext = "NEQ";
        //                            alert(nextValue + " next and curr value  not equal " + currValue + " :: " + currNext);
        //                        }
        var $Dis = 0;
        var $sDist = 0;
        var $Dis11 = "0@0".split('&');
        ...
        //alert($R.rowIndex+"row index :: " + $R.rowIndex);
        ...
        //            $Dis = $Dis;
        //        }
        if (prevValue == currValue && $To[1] == "OS") {
            $Dis = 0;
            ffare = 0;
        }
        // $Dis = $Dis;
    ...
    function onaddExpChange($e) {
        ...
        $R.cells[10].children[0].innerHTML = oldRowTot - oldaddExp + newaddExp;
        $R.cells[10].children[1].value = oldRowTot - oldaddExp + newaddExp;
