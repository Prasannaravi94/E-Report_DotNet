...
var DCR = {
                ...
                if (a[ix].fields != undefined) {
                    ...
                    if (a[ix].fields != undefined) {
                        ...
                        for (_ic in a[ix].fields) {
                            fa = a[ix].fields[_ic];
                            if (fa.m == undefined) fa.m = 0;
                            eTab += "<th>" + ((fa.id == 'cus' && a[ix].nw == 1) ?
                                '<a href="#" class="button button-orange go-bt" style="margin: -3px 3px;height: 15px;padding: 3px 8px;line-height:15px;" onclick="openWin(\'' + ix + '\')">New</a>' : '') + fa.cap + "</th>"; eDtD += '<td>';
                            if (fa.ty.indexOf('ddl') > -1)
                                eDtD += '<div id="ddl_' + ix + fa.id + '" class="' + fa.ty + '" style="width:' + fa.w + 'px;" data-value="' + fa.val + '" data-text="' + fa.df + '" data-inwidth="' + fa.iw + 'px"  data-src="' + fa.src + '" data-adf="' + ((fa.adf != undefined) ? fa.adf : '') + '" data-vf="' + fa.vf + '" data-tf="' + fa.tf + '"></div>';
                            if (fa.ty == 'txt_Tm')
                                eDtD += '<input type="text" id="tx_' + ix + fa.id + '" maxlength="' + fa.mxl + '" class="tmr" style="width:' + fa.w + 'px;"   />';
                            if (fa.ty == 'txt_Dc')
                                eDtD += '<input type="number" id="tx_' + ix + fa.id + '" maxlength="' + fa.mxl + '" min="0" class="' + fa.ty + '" style="width:' + fa.w + 'px;"   />';
                            if (fa.ty.indexOf('button') > -1)
                                eDtD += '<a href="#" class="button go-bt" onclick="DCR.insEntry(\'' + ix + '\')">GO</a>';
                            eDtD += '</td>';
                        }
                        eDtD += '</tr>';
                    }
    ...
    insEntry: function (typ) {
        ...
        for (wky in _C) {
            if (wky == "rem") {
                cust.rem = $('#ddl_' + typ + 'rem').attr('data-text') || "";
                
                //cust.fedbk = {};
                //cust.fedbk.val = $('#ddl_fedBkRmk').attr('data-value') || "";
                //cust.fedbk.txt = $('#ddl_fedBkRmk').attr('data-text') || "";
            }
            else if (wky != "go") {
                ty = _C[wky].ty;
                cust[wky] = {};
                if (ty.indexOf('ddl') > -1) {
                    ...
                    if (wky == "prem" && a != undefined) {
                        ...
                        cust.fedbk = {};                            
                        cust.fedbk.val = $('#ddl_fedBk').attr('data-value') || "0";
                        cust.fedbk.txt = $('#ddl_fedBk').attr('data-text') || "0";
...
ddlcbsel = function () {
    var w = $('#ddl_fedBk').attr('data-text');
    if (w == 'Non Rx') {
        $('.TBH tr').each(function () {
            if (this.id == 'pnlRemarks') {
                $("#pnlRemarks").hide();
                $('#cftprod').hide();
            }
        });
...
setWinVal = function (opnr,win) {
    ...
    if (vals != '') {
        ...
        if (tid == "cftprod") {
            ...
            $('#ddl_fedBk').setItem(sEBval[0], sEBtxt[0]);
            $('#tPrdCusRem').val(sRem[1]);
        ...
        for (il = 0; il < spvs.length - 1; il++) {
            ...
            if (tid == "tProd" || tid == "cftprod") {
                ddl = $(r).find('.Prod');
                 ...
                 if (tid == "cftprod" && il==0) {
                     spt1 = spt[0].split('-');
                     $(ddl).setItem(spv[0], spt1[2]);
                     $(r).find('.tPQ').val(spv[2]);
                 }
                 else if (tid == "cftprod")
                 {
                     $(ddl).setItem(spv[0], spt[0]);
...
svWinVal = function (e) {
    ...
    if (tb.attr("id") == "tProd"){
        $("#tProd").find(".Prod").each(function () {
            ...
            if (Setup.PQtyZro == 1 && $r.find('.tPQ').val() == "") 
            $r.find('.tPQ').val('0');
            if ($r.find('.tPQ').val() == "") {
                alertBox.Show('Enter the Qty',$r.find(".tPQ")); vld = false;
                return false;
            }
            //else if ($r.find('.tPQ').val() == "0") {
            //    alertBox.Show('Not allowed value Zero', $r.find(".tPQ")); vld = false;
            //    return false;
            //}
            ...
            lv += $(this).attr('data-value') + '~' + $r.find('.tPQ').val() + '$0'+ "#";
            tx += $(this).attr('data-text') + ' ( ' + $r.find('.tPQ').val() + ' )';
            if (Setup.DRxQty == 1 && $('.wTab.active').attr('id') == 'idWD') {
                tx += ' ( ' + $r.find('.tPV').val() + " )";
            }
            else {
                tx += ' ( ' +'0'+ " )";
            }
                if (Setup.DRxFed == 1 && $('.wTab.active').attr('id') == 'idWD')
                    tx += ' ( ' + $r.find('.PRx').attr('data-text') + " )";
            tx += ', ';
        })
    }
    else if (tb.attr("id") == "tDetProd") {
        $("#tDetProd").find(".Prod").each(function () {
            $r = $(this).closest('TR');
            if ($(this).attr('data-value') == "") {
                alertBox.Show('Select the Product', $r.find(".disp-selopt")); vld = false;
                return false;
            ...
            lv += $(this).attr('data-value') + '~' +'0'+ '$0'+'#';
            tx += $(this).attr('data-text') + ' ( ' + '0' + " )" + ' ( ' + '0' + " )";
            tx += ', ';
        });
    }

    else if (tb.attr("id") == "cftprod") {

        //var lv = $('#tPrdCusRem').val() + '- ';
        var tx = $('#ddl_fedBk').attr('data-text') + '- ' + $('#tPrdCusRem').val() + '- ';

        

        $("#cftprod").find(".Prod").each(function () {
            $r = $(this).closest('TR');
            ...
            if (w == 'Rx' || w=='') {
                ...
                lv += $(this).attr('data-value') + '~0' + '$' + $r.find('.tPQ').val() + "#";
                tx += $(this).attr('data-text') + ' ( 0 )' + ' ( ' + $r.find('.tPQ').val() + ' )';
    ...
    if ($(win).attr("id") == "wRem") {
        var lv = $('#tCusRem').val();
        var tx = $('#ddl_fedBk').attr('data-text') + ', ' + $('#tCusRem').val() + ', ';
